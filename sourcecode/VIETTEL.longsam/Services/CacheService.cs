namespace Viettel.Services
{
    ...
    public partial class CacheService : ICacheService
    {
        ...
        public void Set(string key, object data, int second_time)
        {
            var policy = new CacheItemPolicy
            {
                AbsoluteExpiration = DateTime.UtcNow + TimeSpan.FromSeconds(second_time)
            };
        ...
        public T CachePerRequest<T>(string cacheKey, Func<T> getCacheItem, CacheTimes minutesToCache)
        {
            var value = Get<T>(cacheKey);
            if (value == null)
            {
                value = getCacheItem();
                Set(cacheKey, value, minutesToCache);
            }

            return value;
        }

        public T CachePerRequest<T>(string cacheKey, Func<T> getCacheItem, int secondToCache)
        {
            var value = Get<T>(cacheKey);
            if (value == null)
            {
                value = getCacheItem();
                Set(cacheKey, value, secondToCache);
            }

            return value;
        }

        public T CachePerRequest<T>(string cacheKey, Func<T> getCacheItem, CacheTimes minutesToCache, bool cacheInDebug)
        {
#if DEBUG
            return getCacheItem();
#else
            return CachePerRequest(cacheKey, getCacheItem, minutesToCache);
#endif
        }
        ...
        public static CacheService Default
        {
            get
            {
                return _cacheService ?? (_cacheService = new CacheService());
            }
        }
        #endregion
    }
