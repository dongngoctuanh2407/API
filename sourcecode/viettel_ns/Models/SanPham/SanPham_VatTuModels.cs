...
namespace VIETTEL.Models
{
    public class SanPham_VatTuModels
    {
        ...
        public static DataTable Get_SanPham_DanhMucGia(String iID_MaSanPham)
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand("SELECT * FROM DM_SanPham_DanhMucGia WHERE iTrangThai=1 AND iID_MaSanPham=@iID_MaSanPham AND iID_MaLoaiHinh=0");
            cmd.Parameters.AddWithValue("@iID_MaSanPham", iID_MaSanPham);
            vR = Connection.GetDataTable(cmd);
            cmd.Dispose();
            return vR;
        }
        public static int GetMa(String iID_MaSanPham,String sKyHieu)
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand("SELECT iID_MaDanhMucGia FROM DM_SanPham_DanhMucGia WHERE iTrangThai=1 AND iID_MaSanPham=@iID_MaSanPham AND sKyHieu=@sKyHieu AND iID_MaLoaiHinh=1");
            cmd.Parameters.AddWithValue("@iID_MaSanPham", iID_MaSanPham);
            cmd.Parameters.AddWithValue("@sKyHieu", sKyHieu);
            return  Convert.ToInt16(Connection.GetValue(cmd,0));
        }
        public static int GetMaCha(String iID_MaSanPham, String sKyHieu)
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand("SELECT iID_MaDanhMucGia_Cha FROM DM_SanPham_DanhMucGia WHERE iTrangThai=1 AND iID_MaSanPham=@iID_MaSanPham AND sKyHieu=@sKyHieu AND iID_MaLoaiHinh=1");
            cmd.Parameters.AddWithValue("@iID_MaSanPham", iID_MaSanPham);
            cmd.Parameters.AddWithValue("@sKyHieu", sKyHieu);
            return Convert.ToInt16(Connection.GetValue(cmd, 0));
        }
        ...
        public static DataTable Get_DanhSachSanPham()
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand();
            String SQL = String.Format("SELECT * FROM DM_SanPham WHERE iTrangThai=1");
            cmd.CommandText = SQL;
            vR = Connection.GetDataTable(cmd);
            cmd.Dispose();
            return vR;
