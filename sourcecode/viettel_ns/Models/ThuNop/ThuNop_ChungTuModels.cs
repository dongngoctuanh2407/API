...
namespace VIETTEL.Models.ThuNop {
    public class ThuNop_ChungTuModels
    {
        /// <summary>
        ...
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand("SELECT * FROM TN_ChungTu WHERE iTrangThai=1 AND iID_MaChungTu=@iID_MaChungTu");
        ...
        /// <summary>
        /// Lấy số chứng từ lớn nhất trong bảng TN_ChungTu
        /// </summary>
        /// <returns></returns>
        public static int GetMaxChungTu()
        {
            int vR;
            SqlCommand cmd = new SqlCommand("SELECT MAX(iSoChungTu) FROM TN_ChungTu WHERE iTrangThai=1");
        ...
        public static int Delete_ChungTu(String iID_MaChungTu, String IPSua, String MaNguoiDungSua)
        {
            ...
            SqlCommand cmd;
            cmd = new SqlCommand("DELETE FROM TN_ChungTuChiTiet WHERE iID_MaChungTu=@iID_MaChungTu");
            cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
        ...
        /// <summary>
        /// Cập nhập trường iID_MaTrangThaiDuyet của bảng TN_ChungTu, TN_ChungTuChiTiet
        /// </summary>
        /// <param name="iID_MaChungTu"></param>
        ...
        public static Boolean Update_iID_MaTrangThaiDuyet(String iID_MaChungTu, int iID_MaTrangThaiDuyet, Boolean TrangThaiTrinhDuyet, String MaND, String IPSua)
        {
            ...
            //Sửa dữ liệu trong bảng TN_ChungTuChiTiet            
            String SQL = "UPDATE TN_ChungTuChiTiet SET iID_MaTrangThaiDuyet=@iID_MaTrangThaiDuyet WHERE iID_MaChungTu=@iID_MaChungTu";
            if (TrangThaiTrinhDuyet)
            {
                SQL = "UPDATE TN_ChungTuChiTiet SET iID_MaTrangThaiDuyet=@iID_MaTrangThaiDuyet, bDongY=0, sLyDo='' WHERE iID_MaChungTu=@iID_MaChungTu";
            }
            cmd = new SqlCommand();
            cmd.Parameters.AddWithValue("@iID_MaTrangThaiDuyet", iID_MaTrangThaiDuyet);
            cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
        ...
        /// <summary>
        /// Cap nhat lai chung tu chi tiet
        /// </summary>
        /// <param name="iID_MaChungTu"></param>
        public static void CapNhapLai_ChungTu(String iID_MaChungTu,String iID_MaDonVi,String MaND)
        {
            SqlCommand cmd = new SqlCommand();
            String SQL =
                String.Format(
                    @"update TN_ChungTuChiTiet set dNgayChungTu=(SELECT TOP 1 dNgayChungTu from TN_ChungTu WHERE iTrangThai=1 and iID_MaChungTu=@iID_MaChungTu),iID_MaDonVi=@iID_MaDonVi,sTenDonVi=@sTenDonVi where iID_MaChungTu=@iID_MaChungTu");
            cmd.CommandText = SQL;
            cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);
            cmd.Parameters.AddWithValue("@iID_MaDonVi", iID_MaDonVi);
            cmd.Parameters.AddWithValue("@sTenDonVi", DonViModels.Get_TenDonVi(iID_MaDonVi,MaND));
            Connection.UpdateDatabase(cmd);
            cmd.Dispose();
        }
        ...
        public static DataTable Get_DanhSachChungTu(String iLoai, String MaND, String SoChungTu,String sThang, String TuNgay, String DenNgay, String MaDonVi, String iID_MaTrangThaiDuyet, int Trang = 1, int SoBanGhi = 0)
        {
            ...
            DataTable vR;
            SqlCommand cmd = new SqlCommand();
            String DK = LuongCongViecModel.Get_DieuKien_TrangThaiDuyet_DuocXem(SharedModels.iID_MaPhanHe, MaND);
            DK += " AND iNamLamViec=@iNamLamViec  AND sID_MaNguoiDungTao=@sID_MaNguoiDungTao ";

            cmd.Parameters.AddWithValue("@iNamLamViec", R["iNamLamViec"]);        
            cmd.Parameters.AddWithValue("@sID_MaNguoiDungTao", MaND);
            if (CommonFunction.IsNumeric(iLoai))
            {
                DK += " AND iLoai = @iLoai";
            ...
            if (String.IsNullOrEmpty(sThang) == false && sThang != "")
            {
                DK += " AND iThang_Quy = @iThang_Quy";
                cmd.Parameters.AddWithValue("@iThang_Quy", sThang);
            }
            ...
            {
                DK += " AND iID_MaDonVi = @MaDonVi";
                cmd.Parameters.AddWithValue("@MaDonVi", MaDonVi);
            }
            String SQL = String.Format("SELECT * FROM TN_ChungTu WHERE iTrangThai=1 AND {0}", DK);
            cmd.CommandText = SQL;
        ...
        public static int Get_DanhSachChungTu_Count(String iLoai, String MaND, String SoChungTu, String TuNgay, String DenNgay, String MaDonVi, String iID_MaTrangThaiDuyet)
        {
            DataTable dtCauHinh = NguoiDungCauHinhModels.LayCauHinh(MaND);
            DataRow R = dtCauHinh.Rows[0];
            ...
            String SQL = String.Format("SELECT Count(*) FROM TN_ChungTu WHERE iTrangThai=1 AND {0}", DK);
            cmd.CommandText = SQL;
            vR = Convert.ToInt32(Connection.GetValue(cmd, 0));
            cmd.Dispose();
            return vR;
        ...
        public static DataTable getDSLoaiThuNop()
        {
            ...
            dr["iLoai"] = "";
            dr["sMoTa"] = "--- Chọn loại thu nộp ---";
            dt.Rows.InsertAt(dr, 0);

            dr = dt.NewRow();            
            dr["iLoai"] = "1";
            dr["sMoTa"] = "Thông tri thu";
            dt.Rows.InsertAt(dr, 1);

            dr = dt.NewRow();
            dr["iLoai"] = "2";
            dr["sMoTa"] = "Tổng hợp thu năm";
