...
namespace VIETTEL.Models
{
    public class NguoiDung_DonViModels
    {
        public static DataTable getList(String MaNguoiDung = "", String MaDonVi = "", int Trang = 1, int SoBanGhi = 0)
        {
            ...
            if (MaDonVi != "" && MaDonVi != Guid.Empty.ToString())
            {
        ...
        public static DataTable getDS(string sMaNguoiDung, int Trang = 1, int SoBanGhi = 0)
        {
            ...
            if (sMaNguoiDung == "")
                SQL = String.Format(
@"SELECT nd.iID_MaNguoiDungDonVi, 
        nd.sMaNguoiDung, 
        nd.iID_MaDonVi, 
        dv.sTen, 
        nd.bPublic, 
        nd.dNgayTao, 
        nd.sID_MaNguoiDungTao 
FROM    NS_NguoiDung_DonVi AS nd, NS_DonVi AS dv 
WHERE   nd.iTrangThai=1 
        AND nd.iNamLamviec=@iNamLamviec 
        AND dv.iNamLamviec_DonVi=@iNamLamviec 
        AND nd.iID_MaDonVi = dv.iID_MaDonVi
ORDER BY nd.iID_MaDonVi");
            else
            {
                SQL = String.Format(
@"SELECT nd.iID_MaNguoiDungDonVi, 
        ...
        nd.iID_MaDonVi, 
        dv.sTen,
        nd.iID_MaDonVi+'-'+dv.sTen as TenHT, 
        nd.bPublic, 
        nd.dNgayTao, 
        nd.sID_MaNguoiDungTao 
FROM    NS_NguoiDung_DonVi AS nd, NS_DonVi AS dv 
WHERE   nd.iTrangThai=1  
        ...
        AND dv.iNamLamviec_DonVi=@iNamLamviec 
        AND nd.iID_MaDonVi = dv.iID_MaDonVi 
        AND nd.sMaNguoiDung=@sMaNguoiDung
ORDER BY nd.iID_MaDonVi
");
            ...
            cmd.CommandText = SQL;
            cmd.Parameters.AddWithValue("@iNamLamViec", NguoiDungCauHinhModels.iNamLamViec);
            var vR = Connection.GetDataTable(cmd);
