...
namespace VIETTEL.Models
{
    ...
    public class LuongCongViecModel
    {
        ...
        public static int iDoiTuongNguoiDung_TroLyPhongBanB2 = 6;
        public static int iDoiTuongNguoiDung_TroLyPhongBanB11 = 7;
        public static Boolean KiemTra_TroLyPhongBan(String MaND)
        {
            ...
            var value = Convert.ToInt32(Connection.GetValue(cmd, 0));
            vR = ( value == iDoiTuongNguoiDung_TroLyPhongBan);
            cmd.Dispose();
            return vR;
        }
        ...
        public static Boolean KiemTra_TroLyPB11_02(String MaND)
        {
            Boolean vR = false;
            SqlCommand cmd = new SqlCommand("SELECT iDoiTuongNguoiDung FROM QT_NguoiDung WHERE sID_MaNguoiDung=@sID_MaNguoiDung");
            cmd.Parameters.AddWithValue("@sID_MaNguoiDung", MaND);
            var value = Convert.ToInt32(Connection.GetValue(cmd, 0));
            vR = (value == iDoiTuongNguoiDung_TroLyPhongBanB2 || value == iDoiTuongNguoiDung_TroLyPhongBanB11 || MaND == "phuongb2");
            cmd.Dispose();
            return vR;
        }        
        ...
        /// <summary>
        /// Hàm lấy danh sách tất cả các trạng thái duyệt được sắp xếp theo số thứ tự
        /// </summary>
        /// <param name="iID_MaPhanHe">Mã phân hệ đang làm việc</param>
        /// <returns></returns>
        public static DataTable Get_dtDSTrangThaiDuyet_All()
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand("SELECT * FROM NS_PhanHe_TrangThaiDuyet WHERE iTrangThai=1 ORDER BY iSTT");
            vR = Connection.GetDataTable(cmd);
            cmd.Dispose();
            return vR;
        }
        ...
        public static Boolean NguoiDung_DuocThemChungTu(int iID_MaPhanHe, String MaND)
        {
            ...
            cmd.Parameters.AddWithValue("@iID_MaPhanHe", iID_MaPhanHe);
            cmd.Parameters.AddWithValue("@iID_MaNhomNguoiDung", iID_MaNhomNguoiDung);
            if (iID_MaNhomNguoiDung != "1-41-12" && iID_MaNhomNguoiDung != "1-41-11")
        ...
        public static int Get_iID_MaTrangThaiDuyetMoi(int iID_MaPhanHe, string username = null)
        {
            var sql = string.IsNullOrWhiteSpace(username) ?
                @"
select iID_MaTrangThaiDuyet from NS_PhanHe_TrangThaiDuyet where iTrangThai=1 and iLoaiTrangThaiDuyet=1 and iID_MaPhanHe=@iID_MaPhanHe

" :
                @"

select iID_MaTrangThaiDuyet from NS_PhanHe_TrangThaiDuyet where iTrangThai=1 and iLoaiTrangThaiDuyet=1 and iID_MaPhanHe=@iID_MaPhanHe and iID_MaNhomNguoiDung=@iID_MaNhomNguoiDung
...
";
            using (var cmd = new SqlCommand(sql))
            {
                ...
                {
                    var iID_MaNhomNguoiDung = BaoMat.LayMaNhomNguoiDung(username);
                    cmd.Parameters.AddWithValue("@iID_MaNhomNguoiDung", iID_MaNhomNguoiDung);
                }

                var vR = Convert.ToInt32(Connection.GetValue(cmd, -1));
                cmd.Dispose();
                return vR;
            }
        }
