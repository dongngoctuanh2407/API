...
namespace VIETTEL.Models
{
    ...
    /// </summary>
    public class DuToan_BangDuLieu : BangDuLieu_E
    {
        protected int _iNamLamViec;
        protected string _iID_MaDonVi,_iChiTapTrung,_iLoai,_sLNS,_iKyThuat,_MaLoai,_iTrangThai;
        ...
        public DuToan_BangDuLieu(String iID_MaChungTu, Dictionary<String, String> arrGiaTriTimKiem, String MaND, String IPSua, String sLNS, String MaLoai, String iLoai, String iChiTapTrung)
        {
            ...
            if (iLoai == "1")
            {
                SQL = "SELECT * FROM DT_ChungTu_TLTH WHERE iID_MaChungTu_TLTH=@iID_MaChungTu AND iTrangThai <> 0";
            }
            //Chung tu nganh ky thuat
            else if (iLoai == "4")
            {
                SQL = "SELECT * FROM DT_ChungTuChiTiet WHERE iID_MaChungTuChiTiet=@iID_MaChungTu AND iTrangThai <> 0";
                sLNS = "1040100";
            }

            else
            {
                SQL = "SELECT * FROM DT_ChungTu WHERE iID_MaChungTu=@iID_MaChungTu AND iTrangThai <> 0";
            }
            ...
            var ctKhoa = Convert.ToBoolean(_dtBang.Rows[0]["bKhoa"]);
            _iID_MaDonVi = Convert.ToString(_dtBang.Rows[0]["iID_MaDonVi"]);
            _iTrangThai = Convert.ToString(_dtBang.Rows[0]["iTrangThai"]);
            ...
            if (iChiTapTrung == "1")
            {
                _dtChiTiet = DuToan_ChungTuChiTietModels.GetChungTuChiTiet_ChiTapTrung(_iID_Ma, arrGiaTriTimKiem, MaND, "'1010000','1020000','1020100','1030100','1091400','1091500'");
                _DuocSuaChiTiet = true;
            }
            else
            {
        ...
        protected void CapNhapHangTongCong(String iLoai = "", String iChiTapTrung = "")
        {
            ...
            if (_iTrangThai == "3" && (_sLNS == "1020100" || _sLNS.StartsWith("2")))
                strDSTruongTien += ",rDonViDN,rTongCong";
            else if(_sLNS.StartsWith("2") && _iTrangThai == "2")
                strDSTruongTien += ",rTuChi_NT";
        ...
        protected void DienDuLieu(String sLNS, String MaLoai, String iLoai, String iKyThuat, String iChiTapTrung)
        {
            ...
            if (_iTrangThai == "3" && (_sLNS == "1020100" || _sLNS.StartsWith("2")))
                arrDSTruongTien = (MucLucNganSachModels.strDSTruongTien + ",rDonViDN,rTongCong").Split(',');
            ...
            if (_arrDuLieu == null)
            {
                ...
                else
                {
                    if (MaLoai == "DuToan")
                    {
                        int count = dtChiTiet.Rows.Count;
                        bool ok = true;
                        CapNhapHangTongCong(iLoai, iChiTapTrung);
                        for (int i = count; i >= 0; i--)
                        {
                ...
                if (iLoai == "4")
                {
                    _nCotSlide = _nCotSlide + _nCotFixed;
                    _nCotFixed = 0;
                }
        ...
        protected void CapNhapDanhSachMaHang()
        {
            ...
            for (int i = 0; i < _dtChiTiet.Rows.Count; i++)
            {
                ...
                {                   
                    MaHang = String.Format("{0}_{1}_{2}", R["iID_MaChungTuChiTiet"], R["iID_MaMucLucNganSach"], R["iID_MaDonVi"]);                   
                }
        ...
        protected override IEnumerable<Cot> LayDSCot()
        {
            ...
            {                
                        ...
                        if (_sLNS == "1040100")
                            items.AddRange(
                                new List<Cot>()
                                {
                                    ...
                                    new Cot(ten: "rDuPhong", tieuDe: "Dự phòng", doRongCot:120, kieuCanLe: "right",kieuNhap: "1"),
                                });
                            ...
                            if (_iTrangThai == "3" && _sLNS == "1020100" )
                                items.AddRange(
                                    new List<Cot>()
                                    {
                                        new Cot(ten: "rDonViDN", tieuDe: "Đơn vị chi tiết", doRongCot:120, kieuCanLe: "right",kieuNhap: "1", chiDoc: true),
                                        new Cot(ten: "rTuChi", tieuDe: "Ngành thẩm định", doRongCot:120, kieuCanLe: "right",kieuNhap: "1"),
                                        new Cot(ten: "rTongCong", tieuDe: "Tổng cộng", doRongCot:120, kieuCanLe: "right",kieuNhap: "1", chiDoc: true),
                                    });                            
                            else
                                items.AddRange(
                                    new List<Cot>()
                ...
                else if (_sLNS.StartsWith("2"))
                { 
                    if (_iTrangThai == "2") { 
                        items.AddRange(
                            new List<Cot>()
                            {
                                new Cot(ten: "rTuChi_NT", tieuDe: $"Số năm {DateTime.Now.Year}", doRongCot:120, kieuCanLe: "right",kieuNhap: "1", chiDoc: true),
                                new Cot(ten: "rChiTaiNganh", tieuDe: "Tự chi tại ngành", doRongCot:120, kieuCanLe: "right",kieuNhap: "1"),
                                new Cot(ten: "rTuChi", tieuDe: "Tự chi", doRongCot:120, kieuCanLe: "right",kieuNhap: "1"),
                                new Cot(ten: "rPhanCap", tieuDe: "Ngành phân cấp bằng tiền", doRongCot:150, kieuCanLe: "right",kieuNhap: "1"),
                            });
                    }
                    else if (_iTrangThai == "3")
                    {
                        items.AddRange(
                                    new List<Cot>()
                                    {
                ...
                else if (_sLNS.StartsWith("4") || _sLNS.StartsWith("5") || _sLNS.StartsWith("8"))
                    items.AddRange(
                        new List<Cot>()
                        {
                            new Cot(ten: "rTuChi", tieuDe: "Tự chi", doRongCot:120, kieuCanLe: "right",kieuNhap: "1"),
            ...
            if (!string.IsNullOrEmpty(_sLNS))
            {
                if (_sLNS.Substring(0, 1) == "8")
                    items.First(x => x.Ten == "sNG").Hien = false;
                if (_sLNS == "1020500" || _sLNS == "1020100" || _sLNS == "1020000" || _sLNS == "1040100" || _iKyThuat == "1" || _sLNS.Substring(0, 1) == "2" || _sLNS.Substring(0, 1) == "4" || _sLNS == "2140100" || _sLNS.Substring(0, 3) == "109")
                {
                    items.AddRange(
                            new List<Cot>()
                            {
