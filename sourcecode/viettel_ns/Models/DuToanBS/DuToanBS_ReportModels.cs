...
SELECT DISTINCT sNG  FROM DT_ChungTuChiTiet_PhanCap WHERE iTrangThai=1 {0} {1} {2} {3}
 ", DK, DKDonVi, DKLNS, DKPhongBan);
        ...
        public static DataTable dtNganh_NhaNuoc(String iID_MaDot, String MaND)
        {
            SqlCommand cmd = new SqlCommand();
            String SQL = "";
            ...
            String iID_MaPhongBan = NganSach_HamChungModels.MaPhongBanCuaMaND(MaND);
            String sTenPB = NguoiDung_PhongBanModels.getTenPhongBan_NguoiDung(MaND);
            bool checkTLTH = LuongCongViecModel.KiemTra_TroLyTongHop(MaND);
            DKPhongBan = ThuNopModels.DKPhongBan_QuyetToan(MaND, cmd);
            //DKDonVi = ThuNopModels.DKDonVi(MaND, cmd);
            DKLNS = ThuNopModels.DKLNS(MaND, cmd, iID_MaPhongBan);
            ...
            DK = " AND sLNS LIKE '207%'";
            DK += " AND iNamLamViec=@iNamLamViec";

            //cmd.Parameters.AddWithValue("@iID_MaDot", iID_MaDot);
            String dsMaChungTu = DKMaChungTuCT(iID_MaDot, MaND);
            DK += " AND iID_MaChungTu IN" + dsMaChungTu;

            cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(MaND));
            SQL = String.Format(@"
SELECT DISTINCT sNG  FROM DTBS_ChungTuChiTiet WHERE iTrangThai=1 {0} {1} {2} {3}
 ", DK, DKDonVi, DKLNS, DKPhongBan);
            cmd.CommandText = SQL;
            DataTable dt = Connection.GetDataTable(cmd);
            DK = "";
            ...
            //Hiep-b11
            bool checkTLPB = LuongCongViecModel.KiemTra_TroLyPhongBan(MaND);
            if (sTenPB != "02" && sTenPB != "11" && checkTLPB)
            {
                DK1 += " AND sMaNguoiQuanLy like '%" + MaND + "%'";
            }
            SQL = String.Format(@"SELECT * FROM NS_MucLucNganSach_Nganh
        ...
        public static DataTable dtNganh(String iID_MaDot, String MaND, String sLNS)
        {
            SqlCommand cmd = new SqlCommand();
            String SQL = "";
...
SELECT DISTINCT sNG  FROM DTBS_ChungTuChiTiet WHERE iTrangThai=1 {0} {1} {2} {3}
 ", DK, DKDonVi, DKLNS, DKPhongBan);
        ...
        public static DataTable dtNganhDot(String toDay, String MaND, String sLNS)
        {
            SqlCommand cmd = new SqlCommand();
            String SQL = "";
            ...
            cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(MaND));
            SQL = String.Format(@"
SELECT DISTINCT sNG  FROM DTBS_ChungTuChiTiet a INNER JOIN (SELECT iID_MaChungTu,dNgayChungTu FROM DTBS_ChungTu WHERE CONVERT(nvarchar,dNgayChungTu,103)>=@toDay) b ON a.iID_MaChungTu = b.iID_MaChungTu WHERE iTrangThai=1 {0} {1} {2} {3}
 ", DK, DKDonVi, DKLNS, DKPhongBan);
            cmd.CommandText = SQL;
            DataTable dt = Connection.GetDataTable(cmd);
            DK = "";
...
SELECT DISTINCT iID_MaDonVi FROM DTBS_ChungTuChiTiet WHERE iTrangThai=1 AND iID_MaDonVi <>''  {0} {1} {2} {3}  AND (rTuChi<>0 OR rHienVat<>0 OR rDuPhong<>0 OR rPhanCap<>0 OR rHangNhap<>0 OR rHangMua<>0)
                                    UNION
                                SELECT DISTINCT iID_MaDonVi FROM DTBS_ChungTuChiTiet_PhanCap WHERE iTrangThai=1 AND iID_MaDonVi <>''  {0} {1} {2} {3}   AND (rTuChi<>0 OR rHienVat<>0 OR rDuPhong<>0 OR rPhanCap<>0 OR rHangNhap<>0 OR rHangMua<>0)
) a
...
WHERE sTenBang='Nganh' AND iTrangThai=1) AND sTenKhoa=@sTenKhoa");
        ...
        public static DataTable dtDonVi_Dot1(String iID_MaDot, String iID_MaPhongBan)
        {
            ...
            {
                DKDot += " AND CONVERT(VARCHAR(24),dNgayChungTu,103)=@MaDot";
        ...
        public static DataTable dtDonVi_Dot2(String iID_MaDotTu, String iID_MaDotDen, String iID_MaPhongBan)
        {
            ...
            String DKDOT = "";
            DK += " AND iID_MaDonVi NOT IN ('')";
            DKDOT += " AND dNgayChungTu BETWEEN @MaDotTu AND @MaDotDen ";
            cmd.Parameters.AddWithValue("MaDotTu", CommonFunction.LayNgayTuXau(iID_MaDotTu));
            cmd.Parameters.AddWithValue("MaDotDen", CommonFunction.LayNgayTuXau(iID_MaDotDen));
            //else
            //{
            //    DK += " ";
            //}
        ...
        //hungpx
        public static String DKMaChungTuCT_Period(String tuNgay, String denNgay, String MaND)
        {
            SqlCommand cmd = new SqlCommand();
            String MaCT = "";
            String SQL = "";
            ...
            if (checkTLTH == true)
            {
                SQL = String.Format(@"SELECT DISTINCT iID_MaChungTu_TLTH, dNgayChungTU 
                    FROM DTBS_ChungTu_TLTH 
                    WHERE iTrangThai = 1 AND iNamLamViec=@iNamLamViec AND CONVERT(nvarchar,dNgayChungTu,103)<=@toDay AND sID_MaNguoiDungTao=@sID_MaNguoiDungTao
                    ORDER BY dNgayChungTu");

                cmd.Parameters.AddWithValue("@fromDay", tuNgay);
                cmd.Parameters.AddWithValue("@toDay", denNgay);
                cmd.Parameters.AddWithValue("@sID_MaNguoiDungTao", MaND);
                cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(MaND));
                cmd.CommandText = SQL;
                DataTable dtMaChungTu_CT = Connection.GetDataTable(cmd);
                String dsMaCT = "";

                foreach (DataRow dtMCT_CT in dtMaChungTu_CT.Rows)
                {
                    String maChungTuTLTH = Convert.ToString(dtMCT_CT["iID_MaChungTu_TLTH"]);

                    string iID_MaChungTu_CTs = Convert.ToString(CommonFunction.LayTruong("DTBS_ChungTu_TLTH", "iID_MaChungTu_TLTH", maChungTuTLTH, "iID_MaChungTu"));
                    String[] maCTArray = iID_MaChungTu_CTs.Split(',');
                    foreach (String MaCTct in maCTArray)
                    {
                        String MaCTc = "'" + MaCTct + "'";
                        iID_MaChungTu_CTs = iID_MaChungTu_CTs.Replace(MaCTct, MaCTc);
                    }
                    dsMaCT += iID_MaChungTu_CTs;
                }
                MaCT = " (" + dsMaCT + ")";
            }
            else
            {
                SQL = String.Format("SELECT iID_MaChungTu FROM DTBS_ChungTu WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec AND CONVERT(nvarchar,dNgayChungTu,103)<=@toDay AND sID_MaNguoiDungTao=@sID_MaNguoiDungTao AND iID_MaPhongBanDich=@iID_MaPhongBan");
                cmd.Parameters.AddWithValue("@iID_MaPhongBan", ReportModels.LayMaPhongBan(MaND));
                cmd.Parameters.AddWithValue("@fromDay", tuNgay);
                cmd.Parameters.AddWithValue("@toDay", denNgay);
                cmd.Parameters.AddWithValue("@sID_MaNguoiDungTao", MaND);
                cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(MaND));
                cmd.CommandText = SQL;
                DataTable dtMaChungTu_CT = Connection.GetDataTable(cmd);
                for (int i = 0; i < dtMaChungTu_CT.Rows.Count; i++)
                {
                    MaCT += "'" + dtMaChungTu_CT.Rows[i]["iID_MaChungTu"] + "'";
                    if (i < dtMaChungTu_CT.Rows.Count - 1)
                        MaCT += " , ";
                }
                MaCT = " (" + MaCT + ")";
            }
        ...
        //hungpx
        public static String DKMaChungTuCT(String iID_MaDot, String MaND)
        {
            SqlCommand cmd = new SqlCommand();
            String MaCT = "";
            String SQL = "";
            ...
            if (checkTLTH == true)
            {
                SQL = String.Format(@"SELECT DISTINCT iID_MaChungTu_TLTH, dNgayChungTU 
                    FROM DTBS_ChungTu_TLTH 
                    WHERE iTrangThai = 1 AND iNamLamViec=@iNamLamViec AND CONVERT(nvarchar,dNgayChungTu,103)=@iID_MaDot AND sID_MaNguoiDungTao=@sID_MaNguoiDungTao
                    ORDER BY dNgayChungTu");
                ...
                String maChungTuTLTH = Convert.ToString(dtMaChungTu_CT.Rows[0]["iID_MaChungTu_TLTH"]);
                ...
                if (String.IsNullOrEmpty(iID_MaChungTu_CTs)) iID_MaChungTu_CTs = "''";
                MaCT = " (" + iID_MaChungTu_CTs + ")";


            }
            else
            {
                SQL = String.Format("SELECT iID_MaChungTu FROM DTBS_ChungTu WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec AND CONVERT(nvarchar,dNgayChungTu,103)=@iID_MaDot AND sID_MaNguoiDungTao=@sID_MaNguoiDungTao AND iID_MaPhongBanDich=@iID_MaPhongBan");
                cmd.Parameters.AddWithValue("@iID_MaPhongBan", ReportModels.LayMaPhongBan(MaND));
                cmd.Parameters.AddWithValue("@iID_MaDot", iID_MaDot);
                cmd.Parameters.AddWithValue("@sID_MaNguoiDungTao", MaND);
                cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(MaND));
                ...
                if (String.IsNullOrEmpty(MaCT)) MaCT = "''";
                MaCT = " (" + MaCT + ")";
            }
            return MaCT;
        }

        public static string GetChungTuList_TheoDot(string iID_MaDot, string username)
        {
            SqlCommand cmd = new SqlCommand();
            String MaCT = "";
            String SQL = "";
            bool checkTLTH = LuongCongViecModel.KiemTra_TroLyTongHop(username);
            if (checkTLTH == true)
            {
                SQL = String.Format(@"SELECT DISTINCT iID_MaChungTu_TLTH, dNgayChungTU 
                    FROM DTBS_ChungTu_TLTH 
            ...
            {
                SQL = String.Format("SELECT iID_MaChungTu FROM DTBS_ChungTu WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec AND CONVERT(nvarchar,dNgayChungTu,103)=@iID_MaDot AND sID_MaNguoiDungTao=@sID_MaNguoiDungTao AND iID_MaPhongBanDich=@iID_MaPhongBan");
                cmd.Parameters.AddWithValue("@iID_MaPhongBan", ReportModels.LayMaPhongBan(username));
                cmd.Parameters.AddWithValue("@iID_MaDot", iID_MaDot);
                cmd.Parameters.AddWithValue("@sID_MaNguoiDungTao", username);
                cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(username));
                cmd.CommandText = SQL;
        ...
        public static string GetChungTuList_DenDot(string iID_MaDot, string username)
        {
            SqlCommand cmd = new SqlCommand();
            String MaCT = "";
            String SQL = "";
        ...
        public static DataTable dtDonVi_Dot_v2(String iID_MaDot, String iID_MaPhongBan, String MaND)
        {
            SqlCommand cmd = new SqlCommand();
            String SQL = "";
            String DK = "";
        ...
        FROM    DTBS_ChungTuChiTiet 
        WHERE   iTrangThai=1 
                AND iID_MaDonVi NOT IN ('') 
                AND iNamLamViec=@iNamLamViec 
                {0} {1} {2} 
                AND iID_MaChungTu IN {3}
        ...
        WHERE   iTrangThai = 1 
                ...
                AND iID_MaDonVi NOT IN ('') 
                AND iID_MaChungTu IN (SELECT iID_MaChungTuChiTiet FROM DTBS_ChungTuChiTiet WHERE iID_MaChungTu IN {3})


        ", DK, DKDonVi, DKPhongBan, dsMaChungTu);
            cmd.CommandText = SQL;
            cmd.Parameters.AddWithValue("@iNamLamViec", ReportModels.LayNamLamViec(MaND));
            DataTable dt = Connection.GetDataTable(cmd);
            cmd.Dispose();
        ...
        /// <param name="username"></param>
        /// <returns></returns>
        public static DataTable dtDonVi_Dot(
            string iID_MaDot,
            string iID_MaPhongBan,
            string username)
        {
            ...
            var sql = @"
SELECT DISTINCT ct.iID_MaDonVi, (ct.iID_MaDonVi + ' - ' + dv.sTen) as TenHT
FROM 
(
    ...
    WHERE   iTrangThai = 1 
            ...
            AND (@iID_MaPhongBan IS NULL OR iID_MaPhongBanDich=@iID_MaPhongBan)
            AND (@iID_MaDonVi IS NULL OR iID_MaDonVi IN (SELECT * FROM F_SplitString2(@iID_MaDonVi)))
            AND iID_MaChungTu IN (SELECT iID_MaChungTuChiTiet FROM DTBS_ChungTuChiTiet WHERE iID_MaChungTu IN (SELECT * FROM F_SplitString2(@iID_MaChungTu)))
) as ct
...
";
            ...
            // dk don vi
            var dtDonVi = NganSach_HamChungModels.DSDonViCuaNguoiDung(username);
            var iID_MaDonVi = dtDonVi.AsEnumerable().Select(x => x.Field<string>("iID_MaDonVi")).Join();
            var iID_MaChungTu = GetChungTuList_TheoDot(iID_MaDot, username);
        ...
        public static DataTable dtLNS_Dot(String iID_MaDot, String iID_MaPhongBan, String iID_MaDonVi, String MaND)
        {
            ...
            var iID_MaPhongBan_guid = NguoiDung_PhongBanModels.getiID_MaPhongBanBysMaNguoiDung(MaND);

            String DKLNS = ThuNopModels.DKLNS(MaND, cmd, iID_MaPhongBan_guid);
            String DKPhongBan = ThuNopModels.DKPhongBan_Dich(MaND, cmd, iID_MaPhongBan);
            String DKDonVi = ThuNopModels.DKDonVi(MaND, cmd);
            ...
            SQL = String.Format(@"SELECT DISTINCT a1.sLNS,a1.sLNS+' - '+b1.sMoTa as TenHT
                                ...
                                FROM DTBS_ChungTuChiTiet 
                                WHERE iID_MaChungTu IN {4} AND iTrangThai=1 AND sLNS!='1040100' AND iNamLamViec=@iNamLamViec {0} {1} {2} {3}
                                ) as a1
                                INNER JOIN 
            ...
            DataTable dt = Connection.GetDataTable(cmd);
            cmd.Dispose();
            //them LNS 1020100 neu co phan cap
            //check co du lieu
            Boolean checkCo1020100 = false;
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (Convert.ToString(dt.Rows[i]["sLNS"]) == "1020100")
                    checkCo1020100 = true;
            }
            if (checkCo1020100 == false)
            {
                SQL = String.Format(@"SELECT SUM(rTuChi) FROM DTBS_ChungTuChiTiet_PhanCap WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec AND sLNS LIKE'1%' {0} {1} {2} {3}
                                            AND iID_MaChungTu IN (SELECT iID_MaChungTuChiTiet 
                                                FROM DTBS_ChungTuChiTiet 
                                                WHERE iID_MaChungTu IN {4})", DK, DKDonVi, "", DKPhongBan, dsMaChungTu);
                cmd.CommandText = SQL;
                Decimal rPhanCap = Convert.ToDecimal(Connection.GetValue(cmd, 0));
                if (rPhanCap > 0)
                {
                    // add 1020100
                    DataRow R = dt.NewRow();
                    R["sLNS"] = "1020100";
                    R["TenHT"] = "1020100 - Nghiệp vu chung";
                    dt.Rows.Add(R);
                }
            }
            return dt;
        ...
        public static DataTable LayDSDot(String iNamLamViec, String MaND)
        {
            ...
            String SQL = String.Format(@"
                    Select DISTINCT CONVERT(nvarchar, dNgayChungTu, 103) as iDotCap, dNgayChungTu
                    FROM DTBS_ChungTu
...
");
        ...
        public static DataTable LayDSDot(String iNamLamViec, String MaND, String sLNS = null)
        {
            ...
            else
            {
                ...
                //truong hop dot co phan cap
                //if (!sLNS.Equals("1040100"))
                //{
                //    DK += " AND sLNS!='1040100'";
                //}
                SQL = String.Format(@"
                    ...
                    WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec
                            ...
                            IN (
                                    SELECT iID_MaChungTu 
                                    FROM DTBS_ChungTuChiTiet 
                                    WHERE iTrangThai=1 AND (rTuChi<>0 OR rPhanCap<>0 OR rDuPhong <>0 OR rHangNhap<>0 OR rHangMua<>0 OR rTonKho<>0 OR rChiTapTrung<>0 OR rHienVat<>0) AND iID_MaDonVi<>'' {0}   
                                ) 
                    ORDER BY dNgayChungTu", DK);
            }
            cmd.Parameters.AddWithValue("@iNamLamViec", iNamLamViec);
        ...
        public static DataTable rptDuToanBS_ChiTieuNganSach(String MaND, String sLNS, String iID_MaDot, String iID_MaDonVi, String iID_MaPhongBan)
        {
            ...
            String dsMaChungTu = DKMaChungTuCT(iID_MaDot, MaND);
            String DKChungTu = "";
            DKChungTu += " AND iID_MaChungTu IN" + dsMaChungTu;
            ...
            String SQL =
                String.Format(@"
                                SELECT SUBSTRING(sLNS,1,1) as sLNS1,
                                        ...
                                        ,SUM(rTuChi/{1}) as rTuChi, SUM(rHienVat/{1}) as rHienVat, SUM(rDuPhong/{1}) as rDuPhong
                                 FROM DTBS_ChungTuChiTiet 
                                 WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec {0} {2} {3} {4}-- AND iBKhac=0
                                 GROUP BY sLNS,sL,sK,sM,sTM,sTTM,sNG,sMoTa,iID_MaDonVi,sTenDonVi,iID_MaPhongBan,sTenPhongBan
                                ...
                                UNION
                                  ...
                                  SELECT SUBSTRING(sLNS,1,1) as sLNS1,
                                        ...
                                        ,SUM(rTuChi/{1}) as rTuChi, SUM(rHienVat/{1}) as rHienVat, SUM(rDuPhong/{1}) as rDuPhong
                                 FROM DTBS_ChungTuChiTiet_PhanCap 
                                 WHERE iTrangThai=1 AND iNamLamViec=@iNamLamViec {0} {2} {3} AND iID_MaChungTu IN (SELECT iID_MaChungTuChiTiet FROM DTBS_ChungTuChiTiet WHERE iTrangThai = 1 {4} ) --AND iBKhac=0
                                 GROUP BY sLNS,sL,sK,sM,sTM,sTTM,sNG,sMoTa,iID_MaDonVi,sTenDonVi,iID_MaPhongBan,sTenPhongBan
                                 HAVING SUM(rTuChi)<>0 OR SUM(rHienVat)<>0 

", DK, donvitinh, DKDonVi, DKPhongBan, DKChungTu);
        ...
        #region dieu kien
        public static String LayDKPhongBan(String MaND, SqlCommand cmd)
        {
            String DKPhongBan = "";
            String sTenPB = NguoiDung_PhongBanModels.getTenPhongBan_NguoiDung(MaND);
            if (sTenPB != "02")
        ...
        public static String LayDKDonVi(String MaND, SqlCommand cmd)
        {
            String DKDonVi = "";
            DataTable dtNĐonVi = NganSach_HamChungModels.DSDonViCuaNguoiDung(MaND);
            for (int i = 0; i < dtNĐonVi.Rows.Count; i++)
