...
namespace VIETTEL.Models.DuToanBS
{
    ...
    /// </summary>
    public class DuToanBSPhanCapBangDuLieu2 : BangDuLieu
    {
        ...
        public DuToanBSPhanCapBangDuLieu2(String iID_MaChungTu, Dictionary<string, string> arrGiaTriTimKiem, string MaND, string IPSua, string sKieuXem)
        {
            ...
            _dtBang = DuToanBSChungTuChiTietModels.LayDanhSachChungTuChiTiet2(_iID_Ma, arrGiaTriTimKiem);
            _iNamLamViec = Convert.ToInt32(CommonFunction.LayTruong("DTBS_ChungTu", "iNamLamViec", iID_MaChungTu, "iNamLamViec"));
            int maTrangThaiDuyet = Convert.ToInt32(CommonFunction.LayTruong("DTBS_ChungTu", "iID_MaChungTu", iID_MaChungTu, "iID_MaTrangThaiDuyet"));
            string maNDTao = Convert.ToString(CommonFunction.LayTruong("DTBS_ChungTu", "iID_MaChungTu", iID_MaChungTu, "sID_MaNguoiDungTao"));
            ...
            string sXauNoiMa;
            if (sLNS.StartsWith("2") || sLNS.Contains("4080300") || sLNS.Contains("4050000"))
            {
            ...
            //Phan cap lan 2
            _dtChiTiet = DuToanBSChungTuChiTietModels.LayChungTuChiTietPhanCap2(dsMaChungTu, arrGiaTriTimKiem, MaND, sLNS, sXauNoiMa, iKyThuat, sKieuXem);
        ...
        protected void DienDuLieu(string sLNS, string sKieuXem, string iKyThuat)
        {
            ...
            if (_arrDuLieu == null)
            {
                ...
                for (int i = 0; i < dtCT.Rows.Count; i++)
                {
                    ...
                    // phan cap ca hang mua + hang nhap + hienvat chung
                ...
                for (int k = 0; k < dtCT.Rows.Count; k++)
                {
                    ...
                    for (int i = pos; i < rang; i++)
                    {
                        if (i < _dtChiTiet.Rows.Count - 1)
                        {
                            ...
                            var truongTien = r["b" + arrDSTruongTien[j]];
                            //if (Convert.ToBoolean(r["b" + arrDSTruongTien[j]]))
                            if (truongTien != DBNull.Value && bool.Parse(truongTien.ToString()))
                            {
