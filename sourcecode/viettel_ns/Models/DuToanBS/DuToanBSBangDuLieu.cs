...
namespace VIETTEL.Models.DuToanBS
{
    ...
    public class DuToanBSBangDuLieu : BangDuLieu
    {
            ...
            string iPSua, string sLNS, string sKieuXem, string iLoai, string iChiTapTrung)
        {
            ...
            if (sLNS != "1040100" && !sLNS.Contains("109") && !sLNS.StartsWith("104"))
            {
            ...
            var namNganSach = row.Field<int>("iID_MaNamNganSach");
            var namLamViec = namNganSach == 1 || namNganSach == 4 || namNganSach == 5
                ? row.Field<int>("iNamLamViec") - 1
                : row.Field<int>("iNamLamViec");

            var locked = NSLockService.Default.CheckLock(namLamViec, "QT", id_phongban);
            if (locked && (namNganSach != 2))
            {
                _ChiDoc = true;
            }
        ...
        protected void DienDuLieu(string sLNS, string sKieuXem, string iLoai, string iKyThuat, string iChiTapTrung)
        {
            ...
            if (_arrDuLieu == null)
            {
                ...
                else
                {
                    ...
                    if (sKieuXem == "DuToan")
                    {
                        ...
                        for (int i = count; i >= 0; i--)
                        {
                            ...
                            //neu ma phong ban nguoi dung khac ma phong ban tao
                            if (maPhongBan != "11" && maPhongBan != Convert.ToString(_dtChiTiet.Rows[i]["iID_MaPhongBan"]) && maPhongBan != Convert.ToString(_dtChiTiet.Rows[i]["iID_MaPhongBanDich"]) && Convert.ToBoolean(_dtChiTiet.Rows[i]["bLaHangCha"]) == false)
                            {
                                isHienThiDong = false;
                            }
                    ...
                    else if (sKieuXem == "PhanCap")
                    {
                        ...
                        for (int i = count; i >= 0; i--)
                        {
                            ...
                            //    }
                            //}
                            if (!String.IsNullOrEmpty(Convert.ToString(dtChiTiet.Rows[i]["iID_MaDonVi"])))
                            {
                                isHienThiDong = true;
                            }
                            ...
                            {
                                //neu ma phong ban nguoi dung khac ma phong ban tao
                                if (maPhongBan != "11" && maPhongBan != Convert.ToString(_dtChiTiet.Rows[i]["iID_MaPhongBanDich"]) && Convert.ToBoolean(_dtChiTiet.Rows[i]["bLaHangCha"]) == false)
                                {
                                    isHienThiDong = false;
                                }
                            }
                    ...
                    else if (sKieuXem == "SapXepNganhCon")
                    {
                        int count = dtChiTiet.Rows.Count - 1;
                        bool isHienThiDong = true;
                        for (int i = count; i >= 0; i--)
                    ...
                    else
                    {
                        ...
                        for (int i = count; i >= 0; i--)
                        {
                            isHienThiDong = true;
                            //neu ma phong ban nguoi dung khac ma phong ban tao
                            if (maPhongBan != "11" && !String.IsNullOrEmpty(Convert.ToString(_dtChiTiet.Rows[i]["iID_MaPhongBan"])) && maPhongBan != Convert.ToString(_dtChiTiet.Rows[i]["iID_MaPhongBan"]) && maPhongBan != Convert.ToString(_dtChiTiet.Rows[i]["iID_MaPhongBanDich"]) && Convert.ToBoolean(_dtChiTiet.Rows[i]["bLaHangCha"]) == false)
                            {
                                isHienThiDong = false;
                            }
                            if (!isHienThiDong)
                ...
                CapNhapDanhSachMaCotFixed(sLNS, iChiTapTrung, iLoai);
        ...
        /// </summary>
        protected void CapNhapDanhSachMaCotFixed(String sLNS, String iChiTapTrung, String iLoai)
        {
            //Khởi tạo các mảng
            ...
            for (int j = 0; j < arrDSTruongTien.Length; j++)
            {
                ...
                {
                    DataRow R = _dtChiTiet.Rows[i];
                    if (Convert.ToBoolean(R["b" + arrDSTruongTien[j]]) && arrDSTruongTien[j].ToString() != "rChiTaiNganh")
                    {
                        _arrCotTienDuocHienThi[arrDSTruongTien[j]] = true;
                        break;
                    } else if (Convert.ToBoolean(R["brPhanCap"]) && arrDSTruongTien[j].ToString() == "rChiTaiNganh")
                    {
                        _arrCotTienDuocHienThi[arrDSTruongTien[j]] = true;
                        break;
                    }
        ...
        protected void CapNhapDanhSachMaCotSlide(string sLNS, string MaLoai, string iLoai, string iKyThuat, string iChiTapTrung)
        {
            ...
            strDSTruong = "sTenDonVi,iID_MaPhongBanDich";
            ...
            string strDSTruongTienTieuDe_Full = DuToanBSModels.strDSTruongTienTieuDe_Full;
        ...
        {
            String strDSTruong = "";
            strDSTruong = "iID_MaDonVi,iID_MaMucLucNganSach,sMaCongTrinh,sMauSac,sFontColor,sFontBold";
            String[] arrDSTruong = strDSTruong.Split(',');
            ...
            for (int i = 0; i < _dtChiTiet.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < _arrDSMaCot.Count; j++)
                {
                    ...
                    else
                    {
                        ...
                        if (sLNS.StartsWith("2") && _arrDSMaCot[j] == "rChiTaiNganh")
                        {
                            okOChiDoc = false;
                        }
                    }
        ...
        protected void CapNhapArrDuLieu(String MaLoai, String iChiTapTrung)
        {
            ...
            for (int i = 0; i < _dtChiTiet.Rows.Count; i++)
            {
                ...
                else
                {
                    ...
                    {
                        //can kiem tra la co phan cap moi hien mau
                        ...
                        double dPhanCap = 0;
                        if (!String.IsNullOrEmpty(Convert.ToString(_dtChiTiet.Rows[i]["rPhanCap"])))
                            dPhanCap = Convert.ToDouble(_dtChiTiet.Rows[i]["rPhanCap"]);
                        double dHienVat = 0;
                        if (!String.IsNullOrEmpty(Convert.ToString(_dtChiTiet.Rows[i]["rHienVat"])))
                            dHienVat = Convert.ToDouble(_dtChiTiet.Rows[i]["rHienVat"]);
