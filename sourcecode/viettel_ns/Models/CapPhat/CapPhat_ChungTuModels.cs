...
namespace VIETTEL.Models
{
    public class CapPhat_ChungTuModels
    {
        ...
        /// <returns></returns>
        public static DataTable LayDanhSachChungTuCuc(String Loai, String sMaPhongBan, String sMaND, String sSoChungTu, String sTuNgay, String sDenNgay,
                                                       ...
                                                       Boolean bLayTheoMaNDTao = false, int iTrang = 1, int iSoBanGhi = 0)
        {
            ...
            if (Loai != Convert.ToString(Guid.Empty) && String.IsNullOrEmpty(Loai) == false && Loai != "")
            {
                DK += " AND iLoai = @Loai";
            ...
            dtView.Sort = "iID_MaTrangThaiDuyet,iSoCapPhat DESC";
            vR = dtView.ToTable();
            cmd.Dispose();            
            return vR;
        }
        ...
        /// <param name="iSoBanGhi"></param>
        /// <returns></returns>
        public static DataTable LayDanhSachChungTuDonVi(String Loai = "", String sSoChungTu = "", String iDM_MaLoaiCapPhat = "", String iID_MaTinhChatCapThu = "", String sLNS = "",
                                                        String iID_MaDonVi = "", String iID_MaTrangThaiDuyet = "", String sTuNgay = "", String sDenNgay = "",
                                                         String sMaND = "", Boolean bLayTheoMaNDTao = true, int iTrang = 1, int iSoBanGhi = 0)
        {
            DataTable vR;
            SqlCommand cmd = new SqlCommand();
            String DK = LuongCongViecModel.Get_DieuKien_TrangThaiDuyet_DuocXem(PhanHeModels.iID_MaPhanHeCapPhat, sMaND);
            ...
            if (String.IsNullOrEmpty(Loai) == false && Loai != "-1")
            {
                DK += " AND iLoai = @iLoai";
            ...
            if (String.IsNullOrEmpty(sLNS) == false && sLNS != "-1")
            {
                DK += " AND sDSLNS LIKE @sLNS";
                cmd.Parameters.AddWithValue("@sLNS", "%" + sLNS + "%");
            }
            else
            {
                DK += " AND sDSLNS IS NOT NULL";
            }
            ...
            String SQL = String.Format("SELECT * FROM CP_CapPhat WHERE iTrangThai=1 AND iID_MaDonVi is not null AND {0}", DK);
            cmd.CommandText = SQL;
        ...
        /// <summary>
        /// Hàm lấy danh sách lựa chọn loại chỉ tiêu dự toán 
        /// được nhập của chứng từ cấp phát
        /// 
        /// </summary>
        /// <returns></returns>
        public static DataTable DS_LoaiDuToan()
        {
            DataTable dt = new DataTable();
            dt.Columns.Add("iID_Loai");
            dt.Columns.Add("TenHT");
        ...
        public static String ThongKeTongSoTien_ChungTu(String iID_MaChungTu)
        {
            ...
            DK += " AND iID_MaCapPhat=@iID_MaChungTu";
            cmd.Parameters.AddWithValue("@iID_MaChungTu", iID_MaChungTu);

            String SQL = String.Format("SELECT SUM(rTuChi) AS TuChi FROM CP_CapPhatChiTiet WHERE {0}", DK);
            cmd.CommandText = SQL;
        ...
        public static Dictionary<string,string> DSTinhchatCap()
        {
            return new Dictionary<string, string>() {
                { "0" , "Cấp có chỉ tiêu" },
                { "1" , "Cấp không có chỉ tiêu" },
                { "2" , "Thu ngân sách" },
                { "3" , "Giảm cấp ngân sách" },
            };
        }
    }
}
