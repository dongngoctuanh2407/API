...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <%
        ...
        dtTrangThai.Dispose();
        DataTable dtChungTuDuyet = DuToan_ChungTuModels.getDanhSachChungTu_TongHopCucDuyet(MaND);
        ...
        Boolean check = LuongCongViecModel.KiemTra_TroLyTongHopCuc(MaND);
        Boolean CheckNDtao=false;
        if (check) CheckNDtao = true;

        DataTable dt = DuToan_ChungTuModels.Get_DanhSachChungTu_Gom_THCuc(iID_MaPhongBan, MaND, sTuNgay, sDenNgay,iID_MaTrangThaiDuyet, CheckNDtao, CurrentPage, Globals.PageSize);

        double nums = DuToan_ChungTuModels.Get_DanhSachChungTu_Gom_THCuc_Count(iID_MaPhongBan, MaND, sTuNgay, sDenNgay, iID_MaTrangThaiDuyet,CheckNDtao, CurrentPage, Globals.PageSize);
...
%>
            <tr>
                <% for (int c = 0; c < 1; c++)
                   {
                       if (z + c < dtChungTuDuyet.Rows.Count)
                       {
                           ...
                           strsTen = 
                                     ...
                                     Convert.ToString(dtChungTuDuyet.Rows[z + c]["sID_MaNguoiDungtao"]) + '-' + Convert.ToString(dtChungTuDuyet.Rows[z + c]["sTenPhongBan"]) + '-' + Convert.ToString(dtChungTuDuyet.Rows[z + c]["sNoiDung"]);
                           MaDonVi = Convert.ToString(dtChungTuDuyet.Rows[z + c]["iID_MaChungTu_TLTH"]);
               ...
               <td align="center" style="width: 40px;">
                    <input type="checkbox" value="<%= MaDonVi %>" <%= strChecked %> check-group="DonVi" id="iID_MaChungTu_TLTH" name="iID_MaChungTu_TLTH" />
                </td>
                <td align="left">
                    <%= strsTen %>
                </td>
        ...
        <table class="mGrid" id="<%= ParentID %>_thList">
            ...
            <%
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (check) 
                    {
                        strEdit = MyHtmlHelper.ActionLink(Url.Action("Edit_Gom", "DuToan_ChungTu", new { iID_MaChungTu = R["iID_MaChungTu_TLTH"], sLNS = sLNS }).ToString(), "<i class='fa fa-pencil-square-o fa-lg color-icon-edit'></i>", "Edit", "", "title=\"Sửa chứng từ\"");
                        strDelete = MyHtmlHelper.ActionLink(Url.Action("Delete_Gom", "DuToan_ChungTu", new { iID_MaChungTu = R["iID_MaChungTu_TLTH"], sLNS = sLNS, MaDotNganSach = MaDotNganSach, ChiNganSach = ChiNganSach }).ToString(), "<i class='fa fa-trash-o fa-lg color-icon-delete'></i>", "Delete", "", "title=\"Xóa chứng từ\"");
                    }
            ...
            <tr <%=strColor %>>
                ...
                <td align="center">
                    <b>
                        <%=MyHtmlHelper.ActionLink(Url.Action("Index", "DuToan_ChungTu", new { ChiNganSach = ChiNganSach, MaDotNganSach = MaDotNganSach,sLNS=sLNS,bTLTH=1,iID_MaChungTu = R["iID_MaChungTu_TLTH"] }).ToString(), "Đợt ngày: " + NgayChungTu, "Detail", "")%></b>
                </td>
                <td align="left">
               ...
               <td align="center">
                    <div onclick="OnInit_CT_NEW(500, 'Duyệt chứng từ');">
                        <%= Ajax.ActionLink("Trình Duyệt", "Index", "NhapNhanh", new { id = "DUTOAN_TRINHDUYETCHUNGTU_GOM_THCUC", OnLoad = "OnLoad_CT", OnSuccess = "CallSuccess_CT", iID_MaChungTu = R["iID_MaChungTu_TLTHCuc"], sLNS = sLNS}, new AjaxOptions { }, new { @class = "buttonDuyet" })%>
                    </div>
                </td>
                <td align="center">
                    <div onclick="OnInit_CT_NEW(500, 'Từ chối chứng từ');">
                        <%= Ajax.ActionLink("Từ Chối", "Index", "NhapNhanh", new { id = "DUTOAN_TUCHOICHUNGTU_GOM_THCUC", OnLoad = "OnLoad_CT", OnSuccess = "CallSuccess_CT", iID_MaChungTu = R["iID_MaChungTu_TLTHCuc"]  , sLNS = sLNS }, new AjaxOptions { }, new { @class = "button123" })%>
                    </div>
                </td>
