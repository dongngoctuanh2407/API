...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <%
        ...
        else
        {
            ...
            DuToan_PhanCapBangDuLieu bang = new DuToan_PhanCapBangDuLieu(iID_MaChungTu, arrGiaTriTimKiem, MaND, IPSua, sLNS, MaLoai);
...
<input type="hidden" id="idDSDonVi" value="<%=strDSDonVi%>"/>
        <%  
            ...
            {
        %>
        <form action="<%=Url.Action("DetailSubmit", "DuToan_PhanCapChungTuChiTiet", new{iID_MaChungTu=iID_MaChungTu,sLNS=sLNS})%>"
        method="post">
        <%
            } %>
         ...
         <%--    <td align="left" style="width: 8%">
                       ...
                       <%-- <td align="left" style="width: 8%">
                            <div onclick="OnInit_CT_NEW(500, 'Tùy chỉnh');">
                                <%= Ajax.ActionLink("Tùy chỉnh", "Index", "NhapNhanh", new { id = "DuToan_PhanCap_TuyChinh", OnLoad = "OnLoad_CT", OnSuccess = "CallSuccess_CT", iID_MaChungTu = iID_MaChungTu }, new AjaxOptions { }, new { @class = "button" })%>
                            </div>
                        </td>--%>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            BangDuLieu_Url_getGiaTri = '<%=Url.Action("get_GiaTri", "DuToan_ChungTuChiTiet")%>';
              BangDuLieu_Url_PhanCap = '<%=Url.Action("Index", "DuToan_ChungTuChiTiet")%>';
