                    ...
                    for(int i=0;i<=dt.Rows.Count-1;i++) {
                        ...
                        <tr <%=classtr %>>
                            ...
                            <td style="padding: 3px 2px;">
                                <%= MyHtmlHelper.ActionLink(urlEdit, "<i class='fa fa-pencil-square-o fa-lg color-icon-edit' title='Sửa'></i>", "Edit", sDanhSachChucNangCam)%>
                                <%if (User.Identity.Name == "admin")
                                                                                                                             { %> |
                                <%= MyHtmlHelper.ActionLink(urlDelete, "<i class='fa fa-trash-o fa-lg color-icon-delete' title='Xóa'></i>", "Delete", sDanhSachChucNangCam)%>
                                <%} %>
                                |
                                <%= MyHtmlHelper.ActionLink(urlSort, "<i class='fa fa-sort fa-lg color-icon-sort' title='Sắp xếp'></i>", "Sort", sDanhSachChucNangCam)%>
                            </td>
                        </tr>
                    <%  }
                    dt.Dispose();
