...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    cmd = new SqlCommand("SELECT iID_MaDanhMuc, sTenKhoa + '_' + sTen AS sTenKhoa FROM DC_DanhMuc " +
                       "WHERE bHoatDong = 1  AND iTrangThai=1 AND iID_MaLoaiDanhMuc = (SELECT iID_MaLoaiDanhMuc " +
                                                                   "FROM DC_LoaiDanhMuc " +
    ...
    cmd = new SqlCommand("SELECT iID_MaDanhMuc, sTenKhoa + '_' + sTen AS sTenKhoa FROM DC_DanhMuc " +
                        "WHERE bHoatDong = 1 AND iTrangThai=1 AND iID_MaLoaiDanhMuc = (SELECT iID_MaLoaiDanhMuc " +
                                                                    "FROM DC_LoaiDanhMuc " +
                                                                    "WHERE sTenBang = 'NhomChinh' AND iTrangThai=1) ORDER BY sTenKhoa");
    ...
    {
        cmd = new SqlCommand("SELECT * FROM DC_DanhMuc WHERE iTrangThai=1 AND iID_MaDanhMuc=@iID_MaDanhMuc");
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            switch (LoaiDanhMuc)
            {
                ...
                case "NhomPhu":
                    ...
                    cmd = new SqlCommand("SELECT iID_MaDanhMucCha FROM DC_DanhMuc WHERE iTrangThai=1 AND iID_MaDanhMuc=@iID_MaDanhMuc");
