...
<%
    ...
    DataTable dt = QLDA_QuyetToanModels.Get_DanhSachQuyetToan_SoPhieu(NamLamViec, sNguoiDung, CurrentPage, Globals.PageSize);

    double nums = QLDA_QuyetToanModels.Get_DanhSachQuyetToan_SoPhieu_Count(NamLamViec,sNguoiDung);
    int TotalPages = (int)Math.Ceiling(nums / Globals.PageSize);
    String strPhanTrang = MyHtmlHelper.PageLinks(String.Format("Trang {0}/{1}:", CurrentPage, TotalPages), CurrentPage, TotalPages, x => Url.Action("Index", "QLDA_QuyetToan", new { NamLamViec = NamLamViec, sNguoiDung = sNguoiDung, page = x }));

    int iDotMoi = 0;
    if (QLDA_QuyetToanModels.Get_Max_Dot(NamLamViec) != "") { 
        iDotMoi = Convert.ToInt32(QLDA_QuyetToanModels.Get_Max_Dot(NamLamViec)) + 1;
    };
    ...
    using (Html.BeginForm("AddNewSubmit", "QLDA_QuyetToan", new { ParentID = ParentID }))
    {
%>
...
<div class="box_tong">
    ...
    <table class='mGrid'>
        ...
        for (i = 0; i < dt.Rows.Count; i++)
        {
            ...
            strEdit = MyHtmlHelper.ActionLink(Url.Action("Edit", "QLDA_QuyetToan", new { iID_MaQuyetToan_SoPhieu = R["iID_MaQuyetToan_SoPhieu"] }).ToString(), "<i class='fa fa-pencil-square-o fa-lg color-icon-edit'></i>", "Edit", "");
            strDelete = MyHtmlHelper.ActionLink(Url.Action("Delete", "QLDA_QuyetToan", new { iID_MaQuyetToan_SoPhieu = R["iID_MaQuyetToan_SoPhieu"] }).ToString(), "<i class='fa fa-trash-o fa-lg color-icon-delete'></i>", "Delete", "");
            ...
            <tr class='<%=sClasstr %>'>
                ...
                <td align="center"><b>
                    <b><%=MyHtmlHelper.ActionLink(Url.Action("Detail", "QLDA_QuyetToan", new { iID_MaQuyetToan_SoPhieu = R["iID_MaQuyetToan_SoPhieu"].ToString() }).ToString(), dNgayLap.ToString(), "Detail", "")%></b></td>
                <td align="center"><b>
                    <b><%=MyHtmlHelper.ActionLink(Url.Action("Detail", "QLDA_QuyetToan", new { iID_MaQuyetToan_SoPhieu = R["iID_MaQuyetToan_SoPhieu"].ToString() }).ToString(), R["iSoQuyetToan"].ToString(), "Detail", "")%></b></td>
