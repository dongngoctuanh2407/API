...
<%
    ...
    DataTable dtNguoiDung = QLDA_TongDuToanModels.Get_NguoiTaoQuyetDinhTongDuToan();
    ...
    DataTable dt = QLDA_TongDuToanModels.Get_DanhSachTongDuToan_QuyetDinh(sSoQuyetDinh, dTuNgay, dDenNgay, sNguoiDung, CurrentPage, Globals.PageSize);

    double nums = QLDA_TongDuToanModels.Get_DanhSachTongDuToan_QuyetDinh_Count(sSoQuyetDinh, dTuNgay, dDenNgay, sNguoiDung);
    int TotalPages = (int)Math.Ceiling(nums / Globals.PageSize);
    String strPhanTrang = MyHtmlHelper.PageLinks(String.Format("Trang {0}/{1}:", CurrentPage, TotalPages), CurrentPage, TotalPages, x => Url.Action("List", "QLDA_TongDuToan", new { sSoQuyetDinh = sSoQuyetDinh, TuNgay = dTuNgay, DenNgay = dDenNgay, sNguoiDung = sNguoiDung, page = x }));

    String strThemMoi = Url.Action("Edit", "QLDA_TongDuToan");

    using (Html.BeginForm("SearchSubmit", "QLDA_TongDuToan", new { ParentID = ParentID }))
    {
%>
...
<div class="box_tong">
    <div class="title_tong">
        <table cellpadding="0" cellspacing="0" border="0" width="100%">
        	<tr>
            	<td>
                    <span>Danh sách quyết định tổng dự toán</span>
                </td>
    ...
    <table class="mGrid">
        ...
        for (i = 0; i < dt.Rows.Count; i++)
        {
            ...
            String strEdit = "";
            String strDelete = "";
            strEdit = MyHtmlHelper.ActionLink(Url.Action("Edit", "QLDA_TongDuToan", new { iID_MaTongDuToan_QuyetDinh = R["iID_MaTongDuToan_QuyetDinh"] }).ToString(), "<i class='fa fa-pencil-square-o fa-lg color-icon-edit'></i>", "Edit", "");
            strDelete = MyHtmlHelper.ActionLink(Url.Action("Delete", "QLDA_TongDuToan", new { iID_MaTongDuToan_QuyetDinh = R["iID_MaTongDuToan_QuyetDinh"], sSoQuyetDinh = R["sSoQuyetDinh"] }).ToString(), "<i class='fa fa-trash-o fa-lg color-icon-delete'></i>", "Delete", "");
            %>
            <tr <%=sClasstr %>>
                <td align="center"><%=R["rownum"]%></td>
                <td align="center">
                    <b><%=MyHtmlHelper.ActionLink(Url.Action("Index", "QLDA_TongDuToan", new { iID_MaTongDuToan_QuyetDinh = R["iID_MaTongDuToan_QuyetDinh"].ToString(), sSoQuyetDinh = R["sSoQuyetDinh"].ToString() }).ToString(), R["sSoQuyetDinh"].ToString(), "Detail", "")%></b>
                </td>   
