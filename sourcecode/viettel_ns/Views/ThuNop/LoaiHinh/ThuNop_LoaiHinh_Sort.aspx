...
<%
    ...
    {
        cmd.CommandText = "SELECT * FROM TN_DanhMucLoaiHinh WHERE iID_MaLoaiHinh_Cha=@iID_MaLoaiHinh_Cha AND iTrangThai = 1 ORDER BY iSTT";
        cmd.Parameters.AddWithValue("@iID_MaLoaiHinh_Cha", iID_MaLoaiHinh_Cha);
    }
    else
    {
        cmd.CommandText = "SELECT * FROM TN_DanhMucLoaiHinh WHERE iID_MaLoaiHinh_Cha = 0 AND iTrangThai = 1 ORDER BY iSTT";
    }
    DataTable dt = Connection.GetDataTable(cmd);

    using (Html.BeginForm("SortSubmit", "ThuNop_LoaiHinh", new { iID_MaLoaiHinh_Cha = iID_MaLoaiHinh_Cha }))
    {   
%>
    ...
    <tr>
        ...
        <td align="left">
            <div style="padding-top: 5px; padding-bottom: 5px; color:#ec3237;">
                ...
                <%=MyHtmlHelper.ActionLink(Url.Action("Index", "TN_LoaiHinh"), "Danh sách chỉ tiêu")%>
            </div>
        </td>
    </tr>
</table>
<div class="box_tong">
    <div class="title_tong">
        <table cellpadding="0" cellspacing="0" border="0" width="100%">
        	<tr>
            	<td>
                	<span>Sắp xếp danh sách loại hình mẫu</span>
                </td>
            </tr>
        </table>
    </div>
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (i == 0)
            {
                strSelected = "selected";
                                ...
                                <option value="<%= Row["iID_MaLoaiHinh"]%>" "<%= strSelected%>"><%= Row["sKyHieu"]%> - <%= Row["sTen"]%></option>
                            <%}
        dt.Dispose();    
                            %>                                 
                    </select>
