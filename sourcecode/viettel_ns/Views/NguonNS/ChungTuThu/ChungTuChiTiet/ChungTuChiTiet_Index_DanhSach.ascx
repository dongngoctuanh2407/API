...
<%
    ...
    string Id_CTThu = Convert.ToString(Request.QueryString["Id_CTThu"]);
...
<div class="box_tong">
    <div id="nhapform">
        <div id="form2">
            ...
            <tr>
                <%
                    for (int j = 0; j < arrDSTruong.Length; j++)
                    {
                        ...
                        string strAttr = string.Format("class='input1_4' onkeypress='jsCTThu_CT_Search_onkeypress(event)' search-control='1' search-field='{1}' style='width:{0}px;height:22px;'", iColWidth - 2, arrDSTruong[j]);
                        if (CoTabIndex)
                        {
            ...
            <iframe id="ifrChiTietChungTu" width="100%" height="538px" src="<%= Url.Action("ChungTuChiTiet_Frame", "ChungTuThu_ChungTuChiTiet", new {Id_CTThu=Id_CTThu})%>"></iframe>
        </div>
    </div>
</div>