...
<body>
    <%
        ...
        String iID_MaCapPhat = Convert.ToString(Request.QueryString["iID_MaChungTu"]);
        DataTable dtTuyChinh = CapPhatModels.getDanhSachTuyChinh();
        SelectOptionList slTuyChinh = new SelectOptionList(dtTuyChinh, "MaLoai", "sTen");     
        dtTuyChinh.Dispose();
        String MaLoai = "";
        if (String.IsNullOrEmpty(MaLoai)) MaLoai = Convert.ToString(CommonFunction.LayTruong("CP_ChungTu", "iID_MaChungTu", iID_MaCapPhat, "MaLoai"));

        using (Html.BeginForm("CapPhatChiTiet_Frame", "CapPhat_ChungTuChiTiet", new { ParentID = ParentID }))
        {
    %>
    <%=MyHtmlHelper.Hidden(ParentID, iID_MaCapPhat, "iID_MaCapPhat", "")%>
