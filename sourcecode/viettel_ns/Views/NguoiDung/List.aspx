...
<%
    ...
    SqlCommand cmd;
    cmd = new SqlCommand("SELECT * FROM QT_NguoiDung WHERE iTrangThai=1 AND ( iID_MaNhomNguoiDung = '1' OR LEFT(iID_MaNhomNguoiDung,2)='1-') ORDER BY iID_MaNhomNguoiDung,sID_MaNguoiDung");
        ...
        <table cellpadding="0" cellspacing="0" border="0" width="100%">
        	<tr>
                ...
                <td align="right" style="padding-right: 10px;">
                    ...
                    <button class="btn btn-primary pull-right" onclick="javascript:location.href='<%: Url.Action("Register", "Account") %>'" ><i class="fa fa-plus" aria-hidden="true"></i>Đăng ký người dùng mới</button>
                </td>
            </tr>
        </table>
    </div>   
...
<%
    ...
    for (i = 0; i < dtNguoiDung.Rows.Count; i++)
    {
        ...
        <tr <%=classtr %>>
            ...
            <td align="center">
                <%=MyHtmlHelper.ActionLink(Url.Action("PasswordReset", "NguoiDung", new { MaNguoiDung = dtNguoiDung.Rows[i]["sID_MaNguoiDung"] }).ToString(), "<i class='fa fa-refresh fa-lg color-icon-edit' title='Thiết lập lại mật khẩu'></i>", "Edit", sDanhSachChucNangCam)%>
            </td>
            <td>
                <%
            ...
            <td align="center">
                <%
                    strTG = "";
                    strTG = MyHtmlHelper.ActionLink(Url.Action("Edit", "NguoiDung", new { MaNguoiDung = dtNguoiDung.Rows[i]["sID_MaNguoiDung"] }).ToString(), "<i class='fa fa-pencil-square-o fa-lg color-icon-edit'></i>", "Edit", sDanhSachChucNangCam);
                    if (strTG != "")
                    {
                        %>
                            <%=strTG %>
                ...
                <%
                    strTG = "";
                    strTG = MyHtmlHelper.ActionLink(Url.Action("Delete", "NguoiDung", new { MaNguoiDung = dtNguoiDung.Rows[i]["sID_MaNguoiDung"], MaNhomNguoiDung = MaNhomNguoiDung }).ToString(), "<i class='fa fa-trash-o fa-lg color-icon-delete'></i>", "Delete", sDanhSachChucNangCam);
                    if (strTG != "")
                    {
                        %>
                            <%=strTG %>
