...
<%
    ...
    String ParentID = "KTCT_TienMat_ChungTu";
    String MaND = User.Identity.Name;
    String sSoChungTuChiTiet = Request.QueryString["SoChungTu"];
    ...
    DataTable dt = KTCT_TienMat_ChungTuModels.Get_DanhSachChungTu(MaND, sSoChungTuChiTiet, dTuNgay, dDenNgay, iID_MaTrangThaiDuyet, CurrentPage, Globals.PageSize);

    double nums = KTCT_TienMat_ChungTuModels.Get_DanhSachChungTu_Count(MaND, sSoChungTuChiTiet, dTuNgay, dDenNgay, iID_MaTrangThaiDuyet);
    ...
    //String strThemMoi = Url.Action("Edit", "KTCT_TienMat_ChungTu");

    using (Html.BeginForm("SearchSubmit", "KTCT_TienMat_ChungTu", new { ParentID = ParentID}))
    {
%>
    ...
    <tr>
        ...
        <td align="left">
            <div style="padding-bottom: 5px; color:#ec3237;">
                ...
                <%=MyHtmlHelper.ActionLink(Url.Action("Index", "KTCT_TienMat_ChungTu"), "Danh sách chứng từ ghi sổ")%>
            </div>
        </td>
    </tr>
</table>
                        ...
                        <table cellpadding="5" cellspacing="5" width="100%">
                            <tr>
                                ...
                                <td class="td_form2_td5">
                                    <div>
                                        <%=MyHtmlHelper.TextBox(ParentID, sSoChungTuChiTiet, "sSoChungTuChiTiet", "", "class=\"input1_2\"", 2)%>
                                    </div>
                                </td>
                            </tr>
                            <tr>
...
<div class="box_tong">
    ...
    <table class="mGrid">
        ...
        for (i = 0; i < dt.Rows.Count; i++)
        {
            ...
            {
                strEdit = MyHtmlHelper.ActionLink(Url.Action("Edit", "KTCT_TienMat_ChungTu", new { iID_MaChungTu = R["iID_MaChungTu"]}).ToString(), "<i class='fa fa-pencil-square-o fa-lg color-icon-edit'></i>", "Edit", "");
                strDelete = MyHtmlHelper.ActionLink(Url.Action("Delete", "KTCT_TienMat_ChungTu", new { iID_MaChungTu = R["iID_MaChungTu"] }).ToString(), "<i class='fa fa-trash-o fa-lg color-icon-delete'></i>", "Delete", "");
            }

            String strURL = MyHtmlHelper.ActionLink(Url.Action("Index", "KTCT_TienMat_ChungTu", new { iID_MaChungTu = R["iID_MaChungTu"]}).ToString(), "<img src='../Content/Themes/images/btnSetting.png' alt='' />", "Detail", null, "title=\"Xem chi tiết chứng từ\"");
            ...
            <tr <%=strColor %>>
                ...
                <td align="center">
                    <b><%=MyHtmlHelper.ActionLink(Url.Action("Index", "KTCT_TienMat_ChungTu", new { iID_MaChungTu = R["iID_MaChungTu"]}).ToString(), Convert.ToString(R["sTienToChungTu"]) + Convert.ToString(R["iSoChungTu"]), "Detail", "")%></b>
                </td>
                ...
                <%if(TrangThaiDuyet==DaDuyet){ %>
                      <div style="float: right; margin-right: 5px;" onclick="OnInit_CT();">      
                        <%= Ajax.ActionLink("Thông tri", "Index", "NhapNhanh", new { id = "KTCT_PHIEUTHU", OnLoad = "OnLoad_CT", OnSuccess = "CallSuccess_CT", Sochungtu = sSoChungTuChiTiet }, new AjaxOptions { }, new { @class = "button_title" })%>                                
                      </div>
                      <%}%>
                </td>
            </tr>
