...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <%
        ...
        DataTable dt = TuLieuLichSuModels.Get_DanhSachHinhAnh(MaKieuTaiLieu, MaDonVi, TuNgay, DenNgay, CurrentPage, Globals.PageSize);
        //int maxHeight = 
        double nums = TuLieuLichSuModels.Get_DanhSachHinhAnh_Count(MaKieuTaiLieu, MaDonVi, TuNgay, DenNgay);
            ...
            <tr bgcolor="#2e6e9e">
                <td style="padding: 10px; width: 10%;">
                 <% if (isModify)
                    {
                        %>
                                <%=MyHtmlHelper.ButtonLink(Url.Action("Edit", "TuLieu_HinhAnh", new { iID_MaKieuTaiLieu = MaKieuTaiLieu }), "&nbsp;&nbsp; Thêm tài liệu &nbsp;&nbsp;", "Edit", "")%>
                        <%
                    }%>
                   
                </td>
                <%
                    using (Html.BeginForm("Search", "TuLieu_HinhAnh", new { ParentID = ParentID }))
                    {   
                %>
                    ...
                    <table class="mGrid">
                        ...
                        <%
                            ...
                            for (i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                String filesUrl = dt.Rows[i]["sFileName"].ToString();
                                String thumsUrl = dt.Rows[i]["sFileNameThums"].ToString();
                                String realThumsUrl = dt.Rows[i]["sFileNameRealThums"].ToString();
                                String strClass = "";
                                if (i % 2 == 0) strClass = "alt";
                                string iID_MaTuLieu = Convert.ToString(R["iID_MaTaiLieu_Anh"]);
                                ...
                                {
                                    strTuChoi = MyHtmlHelper.ActionLink(Url.Action("TuChoi", "TuLieu_HinhAnh", new { iID_MaTuLieu = iID_MaTuLieu }).ToString(), "<img src='../Content/Themes/images/untick.png' alt='' />", "", "");

                                }
                                if (iID_MaTrangThaiDuyet_TrinhDuyet > 0)
                                {
                                    strTrinhDuyet = MyHtmlHelper.ActionLink(Url.Action("TrinhDuyet", "TuLieu_HinhAnh", new { iID_MaTuLieu = iID_MaTuLieu }).ToString(), "<img src='../Content/Themes/images/tick.png' alt='' />", "", "");

                                }
                                if (LuongCongViecModel.NguoiDung_DuocSuaChungTu(PhanHeModels.iID_MaPhanHeTuLieuLichSu, MaND, iID_MaTrangThaiDuyet))
                                {
                                    strEdit = MyHtmlHelper.ActionLink(Url.Action("Edit", "TuLieu_HinhAnh", new { MaTaiLieu = dt.Rows[i]["iID_MaTaiLieu_Anh"] }).ToString(), "<i class='fa fa-pencil-square-o fa-lg color-icon-edit'></i>", "Edit", "");
                                    strDelete = MyHtmlHelper.ActionLink(Url.Action("Delete", "TuLieu_HinhAnh", new { MaTaiLieu = dt.Rows[i]["iID_MaTaiLieu_Anh"] }).ToString(), "<i class='fa fa-trash-o fa-lg color-icon-delete'></i>", "Delete", "");
                                }
                        %>
                        <tr class="<%=strClass %>">
                            <td align="center">
                            ...
                            <td>
                                <%=MyHtmlHelper.ActionLink(Url.Action("Download", "TuLieu_HinhAnh", new { MaTaiLieu = dt.Rows[i]["iID_MaTaiLieu_Anh"] }), Convert.ToString(dt.Rows[i]["sTen"]))%>
                            </td>
                            <td align="center">
                                <div class="jwbox">
                                    <a href="javascript:MediaPlay('player_div<%=i%>')">
                                        <img width="50" height="50" alt="JW Box Image" title="JW Box Image" src="<%=thumsUrl %>" /></a>
                                    <div class="jwbox_hidden">
                                        <div class="jwbox_content" id="player_div<%=i%>" style="overflow: auto;">
                                            <img alt="JW Box Image" title="JW Box Image" src="<%=filesUrl %>" />
                                        </div>
                                        <br />
                                        <div style="text-align: center; height: 30px;">
                                            Bấm phím "<b style="color: Red">Esc</b>" để thoát</div>
                                    </div>
                                    <script type="text/javascript">
                                        document.getElementById("player_div<%=i%>").style.height = (screen.height - 50) + "px";
                                    </script>
                                </div>
                            </td>
                            <td>
                                ...
                                if (tllq != "")
                                {
                                    ...
                                    for (int j = 0; j < arr.Length - 1; j++)
                                    {
                                        if (arr[j] != "")
                                        {
                                            cmd = new SqlCommand("SELECT sTen FROM TL_Anh WHERE iID_MaTaiLieu_Anh=@iID_MaTaiLieu_Anh");
                                            cmd.Parameters.AddWithValue("@iID_MaTaiLieu_Anh", arr[j]);
                                            if (j < arr.Length - 2)
                                            {
                                                %>
                                                <%=MyHtmlHelper.ActionLink(Url.Action("Download", "TuLieu_HinhAnh", new { MaTaiLieu = arr[j] }), Connection.GetValueString(cmd, ""))%>;
                                                <%
                                            }
                                            else
                                            {
