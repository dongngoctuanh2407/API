...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <%
        ...
        DataTable dt = TuLieuLichSuModels.Get_DanhSachVideo(MaKieuTaiLieu, MaDonVi, TuNgay, DenNgay, CurrentPage, Globals.PageSize);

        double nums = TuLieuLichSuModels.Get_DanhSachVideo_Count(MaKieuTaiLieu, MaDonVi, TuNgay, DenNgay);
    ...
    <script type="text/javascript" src="/Scripts/jquery.jwbox.js"></script>
    <link rel="stylesheet" type="text/css" href="/Content/Themes/css/jwbox.css" />
    <script type="text/javascript">
        function MediaPlay(containerId) {
        ...
        function showChild(id) {
            ...
            var img = document.getElementById("tree_img" + objId);
            if (img.alt == "collapse") {
            ...
            if (isShow == true) {
                ...
                for (i = 0; i < parentIdList.length; i = i + 1) {
                    ...
                    if (parentIdArr[parentIdArr.length - 1] == objId) {
                ...
                for (i = 0; i < parentIdList.length; i = i + 1) {
                    ...
                    var parentIdArr = parentIdStr.split("_");
                    //                    alert(objId + "_" + i);
                    //                    alert(parentIdList[i].id.split("parent_id")[1]);
                    if (parentIdArr[parentIdArr.length - 1] == objId) {
                        var childId = parentIdList[i].id.split("parent_id")[1];
            ...
            <tr bgcolor="#2e6e9e">
                <td style="padding: 10px; width: 10%;">
                 <% if (isModify)
                    {
                        %>
                            <%=MyHtmlHelper.ButtonLink(Url.Action("Edit", "TuLieu_Video", new { iID_MaKieuTaiLieu = MaKieuTaiLieu }), "&nbsp;&nbsp; Thêm tài liệu &nbsp;&nbsp;", "Edit", "")%>
                        <%
                    }%>
                   
                </td>
                <%
                    using (Html.BeginForm("Search", "TuLieu_Video", new { ParentID = ParentID }))
                    {   
                %>
                    ...
                    <table class="mGrid">
                        ...
                        <%
                            ...
                            for (i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                String videoUrl = dt.Rows[i]["sURL"].ToString();
                                String strClass = "";
                                if (i % 2 == 0) strClass = "alt";
                                string iID_MaTuLieu = Convert.ToString(R["iID_MaTaiLieu_Video"]);
                                int iID_MaTrangThaiDuyet = Convert.ToInt32(R["iID_MaTrangThaiDuyet"]);

                                int iID_MaTrangThaiDuyet_TuChoi = TuLieuLichSuModels.Get_iID_MaTrangThaiDuyet_TuChoi(MaND, iID_MaTuLieu, "TL_Video", "iID_MaTaiLieu_Video");
                                int iID_MaTrangThaiDuyet_TrinhDuyet = TuLieuLichSuModels.Get_iID_MaTrangThaiDuyet_TrinhDuyet(MaND, iID_MaTuLieu, "TL_Video", "iID_MaTaiLieu_Video");
                                ...
                                {
                                    strTuChoi = MyHtmlHelper.ActionLink(Url.Action("TuChoi", "TuLieu_Video", new { iID_MaTuLieu = iID_MaTuLieu }).ToString(), "<img src='../Content/Themes/images/untick.png' alt='' />", "", "");

                                }
                                if (iID_MaTrangThaiDuyet_TrinhDuyet > 0)
                                {
                                    strTrinhDuyet = MyHtmlHelper.ActionLink(Url.Action("TrinhDuyet", "TuLieu_Video", new { iID_MaTuLieu = iID_MaTuLieu }).ToString(), "<img src='../Content/Themes/images/tick.png' alt='' />", "", "");

                                }
                                if (LuongCongViecModel.NguoiDung_DuocSuaChungTu(PhanHeModels.iID_MaPhanHeTuLieuLichSu, MaND, iID_MaTrangThaiDuyet))
                                {
                                    strEdit = MyHtmlHelper.ActionLink(Url.Action("Edit", "TuLieu_Video", new { MaTaiLieu = dt.Rows[i]["iID_MaTaiLieu_Video"] }).ToString(), "<i class='fa fa-pencil-square-o fa-lg color-icon-edit'></i>", "Edit", "");
                                    strDelete = MyHtmlHelper.ActionLink(Url.Action("Delete", "TuLieu_Video", new { MaTaiLieu = dt.Rows[i]["iID_MaTaiLieu_Video"] }).ToString(), "<i class='fa fa-trash-o fa-lg color-icon-delete'></i>", "Delete", "");
                                }
                        %>
                        <tr class="<%=strClass %>">
                            <td align="center">
                            ...
                            <td>
                                <%=MyHtmlHelper.ActionLink(Url.Action("Download", "TuLieu_Video", new { MaTaiLieu = dt.Rows[i]["iID_MaTaiLieu_Video"] }), Convert.ToString(dt.Rows[i]["sTen"]))%>
                            </td>
                            <td align="center">
                                <div class="jwbox">
                                    <a href="javascript:MediaPlay('player_div<%=i%>')">
                                        <img width="16" height="16" alt="JW Box Image" title="JW Box Image" src="/Content/Themes/images/play_button.png"
                                            onclick="javascript:MediaPlay('player_div<%=i%>');" /></a>
                                    <div class="jwbox_hidden">
                                        <div class="jwbox_content" id="player_div<%=i%>">
                                            <script type="text/javascript">
                                                jwplayer("player_div<%=i%>").setup({
                            ...
                            <td>
                                ...
                                if (tllq != "")
                                {
                                    ...
                                    for (int j = 0; j < arr.Length - 1; j++)
                                    {
                                        if (arr[j] != "")
                                        {
                                            cmd = new SqlCommand("SELECT sTen FROM TL_Video WHERE iID_MaTaiLieu_Video=@iID_MaTaiLieu_Video");
                                            cmd.Parameters.AddWithValue("@iID_MaTaiLieu_Video", arr[j]);
                                            if (j < arr.Length - 2)
                                            {
                                                %>
                                                <%=MyHtmlHelper.ActionLink(Url.Action("Download", "TuLieu_Video", new { MaTaiLieu = arr[j] }), Connection.GetValueString(cmd, ""))%>;
                                                <%
                                            }
                                            else
                                            {
