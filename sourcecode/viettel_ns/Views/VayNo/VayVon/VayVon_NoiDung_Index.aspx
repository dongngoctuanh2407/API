...
<asp:Content ID="Content4" ContentPlaceHolderID="MainContent" runat="server">
    <%
        ...
        String strPhanTrang = MyHtmlHelper.PageLinks(String.Format("Trang {0}/{1}:", CurrentPage, TotalPages), CurrentPage, TotalPages, x => Url.Action("List", new
                                                                                                                                                                    {
                                                                                                                                                                        iID_MaNoiDung = iID_MaNoiDung,
                                                                                                                                                                        sTenNoiDung = sTenNoiDung,
                                                                                                                                                                        page = x
                                                                                                                                                                    }));
        String strThemMoi = Url.Action("EditNoiDung", "VayVon_NoiDung", new { ID = string.Empty });
        DataTable dt = VayNoModels.LayDanhSachNoiDung(iID_MaNoiDung, sTenNoiDung);
        using (Html.BeginForm("SearchSubmitNoiDung", "VayVon_NoiDung", new { ParentID = ParentID, MaNoiDung = iID_MaNoiDung, TenNoiDung = sTenNoiDung }))
        {
    %>
    <div class="box_tong">
        ...
        <table class="mGrid" id="<%= ParentID %>_thList">
            ...
            <%
                for (i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (R["dNgayTao"] != DBNull.Value)
                    {
                        DateTime dtCreate = Convert.ToDateTime(R["dNgayTao"]);
                        shortDate = dtCreate.ToString("dd/MM/yyyy");
                    }

                    string bHoatDong = Convert.ToString(R["bHoatDong"]).Equals("True") ? "Có hiệu lực" : "Không có hiệu lực";
                    strEdit = MyHtmlHelper.ActionLink(Url.Action("EditNoiDung", "VayVon_NoiDung", new { ID = R["ID"] }).ToString(), "<i class='fa fa-pencil-square-o fa-lg color-icon-edit'></i>", "EditNoiDung", "");
                    strDelete = MyHtmlHelper.ActionLink(Url.Action("DeleteNoiDung", "VayVon_NoiDung", new { ID = R["ID"] }).ToString(), "<i class='fa fa-trash-o fa-lg color-icon-delete'></i>", "DeleteNoiDung", "");
            %>
            <tr>
