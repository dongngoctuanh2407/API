...
<%
    ...
    QuyetToan_ChungTuKeToan_BangDuLieu bang = new QuyetToan_ChungTuKeToan_BangDuLieu(iID_MaMucLucNganSach, iQuy, arrGiaTriTimKiem, MaND, IPSua);

    String BangID = "BangDuLieu";
    int Bang_Height = 470;
    int Bang_FixedRow_Height = 50;

    //int iID_MaTrangThaiDuyet_TuChoi = KTCT_TienMat_ChungTuChiTietModels.Get_iID_MaTrangThaiDuyet_TuChoi(MaND, iID_MaDanhMucDuAn);
    //int iID_MaTrangThaiDuyet_TrinhDuyet = KTCT_TienMat_ChungTuChiTietModels.Get_iID_MaTrangThaiDuyet_TrinhDuyet(MaND, iID_MaDanhMucDuAn);
%>
...
<%  
    ...
    <form action="<%=Url.Action("DetailSubmit", "QuyetToan_ChungTuKeToan", new{iID_MaMucLucNganSach=iID_MaMucLucNganSach,iQuy=iQuy})%>" method="post">
    <%
    } %>
