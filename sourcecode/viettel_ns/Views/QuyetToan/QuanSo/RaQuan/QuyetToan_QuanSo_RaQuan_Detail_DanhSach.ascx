...
<%
    ...
    QuyetToan_QuanSo_RaQuan_BangDuLieu bang = new QuyetToan_QuanSo_RaQuan_BangDuLieu(iThang, arrGiaTriTimKiem, MaND, IPSua);

    String BangID = "BangDuLieu";
    int Bang_Height = 470;
    int Bang_FixedRow_Height = 50;
    
    String url = Url.Action("LayThongTinTuLuong", "QuyetToan_QuanSo_ChungTuChiTiet", new { iThang = iThang });
%>
...
<%  
    ...
    <form action="<%=Url.Action("DetailSubmit", "QuyetToan_QuanSo_ChungTuChiTiet", new{iThang=iThang})%>" method="post">
    <%
    } %>
...
%>
    <table width="100%" cellpadding="0" cellspacing="0" border="0" align="right" class="table_form2">
        ...
        <tr>
            <td align="right">
                <%--<input type="button" id="Button1" class="button" onclick="hoighi();" value="<%=NgonNgu.LayXau("Lấy từ lương")%>"/>--%>
                    <button class="btn btn-primary" type="submit" id="Button1" onclick="hoighi();"><i class="fa fa-plus"></i>Lấy từ lương</button>
            ...
            <td align="left" width="90px">
                <%--<input type="button" id="btnLuu" class="button" onclick="javascript:return Bang_HamTruocKhiKetThuc();" value="<%=NgonNgu.LayXau("Thực hiện")%>"/>--%>
                    <button class="btn btn-primary" type="submit" id="btnLuu" onclick="javascript:return Bang_HamTruocKhiKetThuc();"><i class="fa fa-check"></i>Thực hiện</button>
            </td>
            <td>&nbsp;</td>
            <td align="left" width="90px">
                <%--<input class="button" type="button" value="<%=NgonNgu.LayXau("Quay lại")%>" onclick="history.go(-1)" />--%>
                <button class="btn btn-default" type="button" onclick="history.go(-1)"><i class="fa fa-ban"></i>Hủy</button>
            </td>
            <td>&nbsp;</td>
        </tr>
        <tr><td>&nbsp;</td></tr>
...
<script type="text/javascript">
    ...
    function hoighi()
    {
        var ok= confirm('Bạn có chắc chắn lấy thông tin từ lương không?');      
        if(ok==true)
        {       
