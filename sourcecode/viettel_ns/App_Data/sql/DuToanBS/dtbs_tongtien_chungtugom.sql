...
WHERE   iTrangThai=1 
		...
		AND (iID_MaPhongBan = iID_MaPhongBanDich or (iID_MaPhongBanDich = @iID_MaPhongBan and iBKhac = 1))
...
SELECT (SUM(rTuChi) + SUM(rHienVat) + SUM(rHangNhap) + SUM(rHangMua)) as TuChi
--FROM	f_ns_dtbs_phancap(@iNamLamViec,@iID_MaPhongBan, null, @iID_MaChungTu) 

-- longsam fix tốc độ để lấy nhanh hơn
...
WHERE   iTrangThai=1 
			...
			and (
				iID_MaChungTu in (select iID_MaChungTuChiTiet from DTBS_ChungTuChiTiet where iTrangThai=1 and iID_MaPhongBanDich=@iID_MaPhongBan and  iID_MaChungTu in (SELECT * FROM f_split(@iID_MaChungTu)))
			 or( MaLoai=2 
