...
(
	 ...
	 SELECT	iID_MaDonVi		=case when iID_MaDonVi like '[0-9]%' then left(iID_MaDonVi,2) else iID_MaDonVi end
			,NSQP		= case when sLNS like '1%' then sum(rTuChi) else 0 end
			,NSNN		= case when sLNS like '2%' then sum(rTuChi) else 0 end
			,NSDB		= case when sLNS like '3%' then sum(rTuChi) else 0 end
			,KPK		= case when sLNS like '4%' then sum(rTuChi) else 0 end
...
where NSQP + NSNN + NSDB + KPK <> 0 and sTenDonVi is not null
