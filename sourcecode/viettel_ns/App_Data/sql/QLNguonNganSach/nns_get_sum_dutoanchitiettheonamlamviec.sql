...
DECLARE @sSoQuyetDinh nvarchar set @sSoQuyetDinh = null
...
-- (
SELECT ndc.iID_NoiDungChi, ndc.iID_Parent, dt.dNgayQuyetDinh,dt.sSoQuyetDinh,ndc.sMaNoiDungChi, dt.sMaPhongBan, dt.sTenPhongBan, dt.iID_NhiemVu, SUM(dt.SoTien) as iTongTien INTO #TEMP_tong
FROM 
(
	SELECT dt.sSoQuyetDinh, dt.dNgayQuyetDinh, sMaNoiDungChi,sMaPhongBan, concat(sMaPhongBan, ' - ', sTenPhongBan) as sTenPhongBan, dtct.iID_NhiemVu, dtct.SoTien
...
GROUP BY ndc.iID_NoiDungChi, ndc.iID_Parent, ndc.sMaNoiDungChi, dt.sMaPhongBan, dt.sTenPhongBan, dt.iID_NhiemVu, dt.dNgayQuyetDinh, dt.sSoQuyetDinh
-- ) dtTheoDV
...
UNION 
SELECT ndc.iID_NoiDungChi, ndc.iID_Parent, null as dNgayQuyetDinh, null as sSoQuyetDinh, ndc.sMaNoiDungChi, null as sMaPhongBan, null as sTenPhongBan, null as iID_NhiemVu, 0 as iTongTien
...
#TEMP_tong1 T  
  inner join (  
             select RootID, sSoQuyetDinh, dNgayQuyetDinh, sMaPhongBan, sTenPhongBan, iID_NhiemVu, 
                    sum(iTongTien) as AmountIncludingChildren  
             from C  
						 WHERE C.iTongTien is not null
             group by RootID, sSoQuyetDinh, dNgayQuyetDinh, sMaPhongBan, sTenPhongBan, iID_NhiemVu   
             ) as S  
