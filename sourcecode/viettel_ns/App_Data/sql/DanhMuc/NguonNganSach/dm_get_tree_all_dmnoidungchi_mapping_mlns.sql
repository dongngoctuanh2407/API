...
SELECT dmndc.*, 
		CAST((case when tbl1.iID_NoiDungChi is not null and tblCheck.iID_NoiDungChi is not null then N'Đã map'
				...
				case when tbl1.iID_NoiDungChi is not null and tblCheck.iID_NoiDungChi is not null then 1
				when tbl1.iID_NoiDungChi is not null and tblCheck.iID_NoiDungChi is null then 0
				else null end as isMap ,
...
LEFT JOIN (select distinct iID_NoiDungChi from NNS_NDChi_MLNS where NamLamViec = @iNamLamViec) tblCheck on dmndc.iID_NoiDungChi = tblCheck.iID_NoiDungChi
WHERE 1 = 1
