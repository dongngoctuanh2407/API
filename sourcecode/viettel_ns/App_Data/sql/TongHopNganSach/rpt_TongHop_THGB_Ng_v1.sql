...
FROM (
	...
	SELECT  sLNS = '1040200',sL,sK,sM,sTM,sTTM,sNG,iId_MaDonVi,
	...
	SELECT  sLNS = '1040300',sL,sK,sM,sTM,sTTM,sNG,iId_MaDonVi,
            rTuChi=	SUM(rHangMua)
    FROM f_ns_table_chitieu(@iNamLamViec,@iID_MaDonVi,@iID_MaPhongBan,1,GETDATE(),1,null)
	where sLNS like '104%'
    GROUP BY sLNS,sL,sK,sM,sTM,sTTM,sNG,iId_MaDonVi
...
--left join (select distinct sLNS as id,sL AS id1,sK AS id2,sM AS id3 ,sTM AS id4,sTTM AS id5,sNG AS id6,sMoTa from NS_MucLucNganSach where iTrangThai=1 and iNamLamViec=@iNamLamViec and sL='' and sLNS=sXauNoiMa) as mlns on (dt.sLNS=mlns.id AND dt.sL=mlns.id1 AND dt.sK=mlns.id2 AND dt.sM=mlns.id3 AND dt.sTM=mlns.id4 AND dt.sTTM=mlns.id5 AND dt.sNG=mlns.id6)


 --lay mota lns
--left join 
--(select sXauNoiMa as mlns_id, sMoTa from dbo.f_mlns_mota2(@iNamLamViec)) as mlns
--on mlns.mlns_id=DT.sLNS+'-'+DT.sL+'-'+DT.sK+'-'+DT.sM+'-'+DT.sTM+'-'+DT.sTTM+'-'+DT.sNG
...
where  
		...
		AND sNG <> ''		
--group by sLNS1, sLNS3, sLNS5,sLNS,sL,sK,sM,sTM,sTTM,sNG, mlns.sMoTa, iID_MaDonVi
group by sLNS1, sLNS3, sLNS5,sLNS,sL,sK,sM,sTM,sTTM,sNG, iID_MaDonVi
Having sum(CT_DauNam) <> 0 OR
        sum(CT_BoSung) <> 0 OR
        sum(QT_rTuChi) <> 0
ORDER BY iID_MaDonVi,sM,sTM,sTTM,sNG
