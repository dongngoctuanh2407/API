...
declare @iID_MaDonVi nvarchar(max)					set @iID_MaDonVi='10'
...
select  
	sLNS1 = CONVERT(nvarchar(3),sLNS1),
...
(
	...
	SELECT  
		--sLNS1	=LEFT(sLNS,1),
		sLNS1	=CASE 
					WHEN sLNS LIKE '105%' THEN 1
					WHEN sLNS LIKE '1%' THEN LEFT(sLNS,3) ELSE LEFT(sLNS,1) END,
	...
	(
		...
		*/
		...
		--1. Hợp thức cấp
		SELECT  sLNS,
				iID_MaDonVi = LEFT(HT_CAP.iID_MaDonVi,2), 
		...
		FROM (
			...
			--FROM f_ns_chitieu_full5(@iNamLamViec,@iID_MaDonVi,@iID_MaPhongBan,1,null,GETDATE(),1,null)
			FROM @Temp1
			...
			SELECT  sLNS, iId_MaDonVi,
					rTuChi=	CASE    
						WHEN sLNS LIKE '104%' then SUM(rTuChi)
						ELSE SUM(rTuChi+rHangNhap+rHangMua) END 
			--FROM f_ns_chitieu_full5(@iNamLamViec,@iID_MaDonVi,@iID_MaPhongBan,5,null,GETDATE(),1,null)
			FROM @Temp2
			GROUP BY sLNS1,sLNS3,sLNS5,sLNS, iId_MaDonVi
		...
		SELECT  sLNS,
				iID_MaDonVi = LEFT(HT_QuyetToan.iID_MaDonVi,2), 
				0 as HT_Cap,
		...
		*/
		...
		SELECT  sLNS,
				iID_MaDonVi = LEFT(CT_NAMTRUOC.iID_MaDonVi,2),
				0 as HT_Cap,
				0 as HT_QuyetToan,
		...
		-- du toan dau nam
			...
			--FROM    f_ns_chitieu_full5(@iNamLamviec, @iID_MaDonVi, @iID_MaPhongBan, 4,null,GETDATE(),1,null)
			FROM @Temp3
		...
		*/
		-- du toan
		SELECT  sLNS,
				iID_MaDonVi = LEFT(CT.iID_MaDonVi,2),
				0 as HT_Cap,
				0 as HT_QuyetToan,
				0 as CT_NamTruoc,
		...
		FROM (
			...
			SELECT      
					...
					rTuChi=	CASE    
							...
							ELSE SUM(rTuChi+rHangNhap+rHangMua) END,
					0 as rBS
			--FROM    f_ns_chitieu_full5(@iNamLamviec, @iID_MaDonVi, @iID_MaPhongBan, 2,null,GETDATE(),1,1)
			FROM @Temp4
			GROUP BY sLNS,iID_MaDonVi
			HAVING	SUM(rTuChi)<>0
			...
			--FROM    f_ns_chitieu_full5(@iNamLamviec, @iID_MaDonVi, @iID_MaPhongBan, 2,null,GETDATE(),1,2)
			FROM @Temp5
			GROUP BY sLNS,iID_MaDonVi
			HAVING	SUM(rTuChi)<>0 
...
left join (select distinct sXauNoiMa as id,sMoTa from NS_MucLucNganSach where iTrangThai=1 and iNamLamViec=@iNamLamViec and sL='' and LEN(sXauNoiMa)<=3) as mlns on dt.sLNS=mlns.id

where 
        sLNS1 in (104,109,2,3,4)
        --AND sLNS not in(1050000)
        AND (@iID_MaDonVi IS NULL OR iID_MaDonVi IN (SELECT * FROM F_Split(@iID_MaDonVi)))
		--and slns like '102%'
group by sLNS1,sMoTa,iID_MaDonVi
having	SUM(HT_Cap)<>0 
