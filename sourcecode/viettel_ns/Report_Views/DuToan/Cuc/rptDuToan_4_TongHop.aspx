...
<body>
    <%
        ...
        int trinhky = (int)ViewData["trinhky"];
        ...
        DataTable dtTo = rptDuToan_4_TongHopController.DanhSachToIn(MaND, "", ToSo,sLNS,iID_MaPhongBan);
        ...
        if (PageLoad == "1")
        {
            for (int i = 0; i < arrMaTo.Length; i++)
            {
                arrView[i] =
                    String.Format(
                        @"/rptDuToan_4_TongHop/viewpdf?ToSo={0}&MaND={1}&Nganh={2}&sLNS={3}&iID_MaPhongBan={4}&trinhky={5}",
        ...
        using (Html.BeginForm("EditSubmit", "rptDuToan_4_TongHop", new { ParentID, trinhky }))
        {
    %>
    <div class="box_tong">
        <div class="title_tong">
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                    <td>
                        <span>Báo cáo dự toán chi ngân sách khác năm
                            <%=iNamLamViec%>
                            (Phần phụ lục 6a- C)</span>
                    </td>
                </tr>
            </table>
        </div>
        <div id="Div1">
         ...
         $("input:checkbox[check-group='To']").each(function (i) {
            ...
            function Chon() {
                ...
                var url = unescape('<%= Url.Action("Ds_DonVi?ParentID=#0&Nganh=#1&ToSo=#2&sLNS=#3&iID_MaPhongBan=#4&trinhky=#5", "rptDuToan_4_TongHop") %>');
                ...
                url = unescape(url.replace("#5","<%= trinhky %>"));
    ...
    %>
    <%--<div>
        <%=MyHtmlHelper.ActionLink(Url.Action("ExportToExcel", "rptDuToan_4_TongHop", new { MaND = MaND, Nganh = "", ToSo = MaTo, iID_MaPhongBan = iID_MaPhongBan, sLNS = sLNS }), "Xuất ra Excels")%>
    </div>--%>
</body>
</html>
