...
<%
    ...
    DataTable dtTo = rptDuToan_1050000_TongHop_ChonToController.DanhSachToIn(MaND, Nganh, ToSo);
    ...
    if (PageLoad == "1")
    {
        ...
        for (int i = 0; i < arrMaTo.Length; i++)
        {
            arrView[i] =
                String.Format(
                    @"/rptDuToan_1050000_TongHop_ChonTo/viewpdf?ToSo={0}&MaND={1}&trinhky={2}",
                    arrMaTo[i], MaND, trinhky);
            Chuoi += arrView[i];
    ...
    using (Html.BeginForm("EditSubmit", "rptDuToan_1050000_TongHop_ChonTo", new { ParentID, trinhky }))
    {
    %>   
...
for (int i = 0; i < dtTo.Rows.Count; i = i + SoCot)
{
    ...
    <%}
         ...
         %>
    <div>    
       <%=MyHtmlHelper.ActionLink(Url.Action("Download", "rptDuToan_1050000_TongHop_ChonTo", new { Nganh, ToSo, trinhky }), "Xuất báo cáo ra file Excel")%>
    </div>
</body>
</html>
