...
<body>
    <%
        ...
        DataTable dtTo = rptDuToan_2_B8_TongHopController.DanhSachToIn(MaND, "", ToSo,sLNS,iID_MaPhongBan);
        ...
        if (PageLoad == "1")
        {
            for (int i = 0; i < arrMaTo.Length; i++)
            {
                arrView[i] =
                    String.Format(
                        @"/rptDuToan_2_B8_TongHop/viewpdf?ToSo={0}&MaND={1}&Nganh={2}&sLNS={3}&iID_MaPhongBan={4}",
                        arrMaTo[i], MaND, "", sLNS, iID_MaPhongBan);
                Chuoi += arrView[i];
        ...
        using (Html.BeginForm("EditSubmit", "rptDuToan_2_B8_TongHop", new { ParentID = ParentID, iID_MaPhongBan }))
        {
    %>
    <div class="box_tong">
        ...
        <div id="Div1">
         ...
         $("input:checkbox[check-group='To']").each(function (i) {
            ...
            function Chon() {
                ...
                var url = unescape('<%= Url.Action("Ds_DonVi?ParentID=#0&Nganh=#1&ToSo=#2&sLNS=#3&iID_MaPhongBan=#4", "rptDuToan_2_B8_TongHop") %>');
    ...
    %>
    <%--<div>
        <%=MyHtmlHelper.ActionLink(Url.Action("ExportToExcel", "rptDuToan_2_B8_TongHop", new { MaND = MaND, Nganh = "", ToSo = MaTo, iID_MaPhongBan = iID_MaPhongBan, sLNS = sLNS }), "Xuất ra Excels")%>
    </div>--%>
</body>
</html>
