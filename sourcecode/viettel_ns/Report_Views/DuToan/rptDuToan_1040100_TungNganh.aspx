...
<body>
    <%
        ...
        dtNguonBD.Dispose();
        DataTable dtTo = rptDuToan_1040100_TungNganhController.DanhSachToIn(MaND, Nganh, ToSo,sLNS,iID_MaPhongBan);
        ...
        if (PageLoad == "1")
        {
            for (int i = 0; i < arrMaTo.Length; i++)
            {
                arrView[i] =
                    String.Format(
                        @"/rptDuToan_1040100_TungNganh/viewpdf?ToSo={0}&MaND={1}&Nganh={2}&sLNS={3}&iID_MaPhongBan={4}",
                        arrMaTo[i], MaND, Nganh,sLNS,iID_MaPhongBan);
                Chuoi += arrView[i];
        ...
        using (Html.BeginForm("EditSubmit", "rptDuToan_1040100_TungNganh", new { ParentID = ParentID }))
        {
    %>
    <div class="box_tong">
        ...
        <div id="Div1">
         ...
         $("input:checkbox[check-group='To']").each(function (i) {
            ...
            function Chon() {
                ...
                var url = unescape('<%= Url.Action("Ds_DonVi?ParentID=#0&Nganh=#1&ToSo=#2&sLNS=#3&iID_MaPhongBan=#4", "rptDuToan_1040100_TungNganh") %>');
