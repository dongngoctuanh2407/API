...
<body>
    <%
        String ParentID = "DuToan";
         ...
         DataTable dtDonVi = DuToan_ReportModels.dtNganh(MaND,"1040100,1020100");
        ...
        dtCapTongHop.Rows.RemoveAt(2);
         ...
         if (PageLoad == "1")
         {
             ...
             for (int i = 0; i < arrMaDonVi.Length; i++)
             {
                 arrView[i] =
                     String.Format(
                         @"/rptDuToan_BaoDam_TungDonVi_Bia/viewpdf?iID_MaDonVi={0}&MaND={1}&iCapTongHop={2}",
         ...
         using (Html.BeginForm("EditSubmit", "rptDuToan_BaoDam_TungDonVi_Bia", new {ParentID=ParentID}))
        {
    %>
...
for (int i = 0; i < dtDonVi.Rows.Count; i = i + SoCot)
{
    ...
    <%} %>
    <div>
        <%=MyHtmlHelper.ActionLink(Url.Action("ExportToExcel", "rptDuToan_BaoDam_TungDonVi", new { MaND = MaND, iID_MaDonVi = iID_MaDonVi, iCapTongHop = iCapTongHop }), "Xuất ra Excels")%>
    </div>
</body>
</html>
