...
<body>
    <%
        ...
        DataTable dtDonVi = DuToan_ReportModels.dtDonVi(MaND, "2060101");
        ...
        if (PageLoad == "1")
        {
            ...
            for (int i = 0; i < arrMaDonVi.Length; i++)
            {
                arrView[i] =
                    String.Format(
                        @"/rptDuToan_206_TungDonVi/viewpdf?iID_MaDonVi={0}&MaND={1}",
                        arrMaDonVi[i], MaND);
                Chuoi += arrView[i];
        ...
        using (Html.BeginForm("EditSubmit", "rptDuToan_206_TungDonVi", new { ParentID = ParentID }))
        {
    %>
    <div class="title_tong">
        <table cellpadding="0" cellspacing="0" border="0" width="100%">
            <tr>
                <td>
                    <span>Báo cáo dự toán chi ngân sách nhà nước giao (phần chi người có công) năm
                        <%=iNamLamViec %>
                    </span>
                </td>
            </tr>
...
for (int i = 0; i < dtDonVi.Rows.Count; i = i + SoCot)
{
        ...
        <script type="text/javascript">
                ...
                var arrDonVi=iID_MaDonVi.split(',');
                var count = arrDonVi.length;
...
form.setAttribute("action", "viewpdf");
form.setAttribute("target", "_blank");

var hiddenField = document.createElement("input"); 
hiddenField.setAttribute("type", "hidden");
hiddenField.setAttribute("name", "iID_MaDonVi");
hiddenField.setAttribute("value", arrDonVi[i]);
form.appendChild(hiddenField);

hiddenField = document.createElement("input"); 
hiddenField.setAttribute("type", "hidden");
hiddenField.setAttribute("name", "MaND");
hiddenField.setAttribute("value", '<%=MaND %>');
form.appendChild(hiddenField);
