...
<body>
    <%
        ...
        if(String.IsNullOrEmpty(iID_MaChungTu))
            iID_MaPhongBan = Convert.ToString(ViewData["iID_MaChungTu"]);
        ...
        dtNguonBD.Dispose();
        rptDuToan_1040100_TungNganh_TrinhKyController rpt = new rptDuToan_1040100_TungNganh_TrinhKyController();
        DataTable dtTo = rpt.DanhSachToIn(MaND, Nganh, ToSo, sLNS, iID_MaPhongBan);
        ...
        if (PageLoad == "1")
        {
            for (int i = 0; i < arrMaTo.Length; i++)
            {
                arrView[i] =
                    String.Format(
                        @"/rptDuToan_1040100_TungNganh_trinhky/viewpdf?ToSo={0}&MaND={1}&Nganh={2}&sLNS={3}&iID_MaPhongBan={4}&iID_MaChungTu={5}",
                        arrMaTo[i], MaND, Nganh,sLNS,iID_MaPhongBan,iID_MaChungTu);
                Chuoi += arrView[i];
        ...
        using (Html.BeginForm("EditSubmit", "rptDuToan_1040100_TungNganh_trinhky", new { ParentID = ParentID, iID_MaChungTu = iID_MaChungTu }))
        {
    %>
    <div class="box_tong">
        ...
        <div id="Div1">
                            ...
                            <div style="margin-top: 10px;">
                                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                    <tr>
                                        ...
                                        <%-- <td>
                                            <input id="btn-print" type="button" class="button" value="In báo cáo" />
                                        </td>--%>
                                        <td style="width: 1%">
                                            &nbsp;
                                        </td>
         ...
         $("input:checkbox[check-group='To']").each(function (i) {
            ...
            function Chon() {
                ...
                var url = unescape('<%= Url.Action("Ds_DonVi?ParentID=#0&Nganh=#1&ToSo=#2&sLNS=#3&iID_MaPhongBan=#4&iID_MaChungTu=#5", "rptDuToan_1040100_TungNganh_TrinhKy") %>');
                ...
                url = unescape(url.replace("#5","<%=iID_MaChungTu %>"));
                $.getJSON(url, function (data) {
                    document.getElementById("<%= ParentID %>_tdDonVi").innerHTML = data;
                        ...
                        //    var nganh = $("#BaoCaoNganSachNam_Nganh").val();
                        //    var iID_MaPhongBan = $("#BaoCaoNganSachNam_iID_MaPhongBan").val();
                        //    var ToSo = $("#")
                        //    var sLNS = $("#BaoCaoNganSachNam_sLNS").val();

                        //    var url = "/rptDuToan_1040100_TungNganh/viewpdf?ToSo={0}&MaND={1}&Nganh={2}&sLNS={3}&iID_MaPhongBan={4}";
    ...
    %>
    <div>
        <%=MyHtmlHelper.ActionLink(Url.Action("Download", "rptDuToan_1040100_TungNganh", new { MaND = MaND, Nganh = Nganh, ToSo = MaTo, iID_MaPhongBan = iID_MaPhongBan, sLNS = sLNS }), "Xuất ra Excels")%>
    </div>
</body>
</html>
