...
<body>
    <%
        String ParentID = "DuToan";
         ...
         String iID_MaNganh = Convert.ToString(ViewData["iID_MaNganh"]);
         DataTable dtNganh = DuToan_ReportModels.dtNganh(MaND,"1040100,1020100,1020000");

         if (String.IsNullOrEmpty(iID_MaNganh))
         {
             if (dtNganh.Rows.Count > 0)
             {
                 iID_MaNganh = Convert.ToString(dtNganh.Rows[0]["iID_MaNganh"]);
             }
             else
             {
                 iID_MaNganh = Guid.Empty.ToString();
             }
         }
         dtNganh.Dispose();
         String[] arrMaNganh = iID_MaNganh.Split(',');
         ...
         if (String.IsNullOrEmpty(PageLoad))
             PageLoad = "0";
         if (String.IsNullOrEmpty(iID_MaNganh)) PageLoad = "0";
         if (PageLoad == "1")
         {
             ...
             for (int i = 0; i < arrView.Length; i++)
             {
                 arrView[i] =
                     String.Format(
                         @"/rptDuToan_1040100_TungDonVi_KiemSoLieu/viewpdf?iID_MaNganh={0}&MaND={1}&iID_MaDonVi={2}",
                         iID_MaNganh, MaND,iID_MaDonVi);
                 Chuoi += arrView[i];
                 if (i < arrMaNganh.Length - 1)
                     Chuoi += "+";
             }
         ...
         using (Html.BeginForm("EditSubmit", "rptDuToan_1040100_TungDonVi_KiemSoLieu", new {ParentID=ParentID}))
        {
    %>
     <div class="title_tong">
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                    <td>
                        <span>Báo cáo kiểm số liệu (phần ngân sách bảo đảm) năm <%=iNamLamViec %>
                           </span>
                    </td>
                            ...
                            <table class="mGrid" style="width: 100%">
                                <tr>
                                    <th align="center" style="width: 40px;">
                                        <input type="checkbox" id="Checkbox1" onclick="CheckAllDV(this.checked)" />
                                    </th>
...
String sTenDonVi = "", MaDonVi = "", Checked = "";
for (int i = 0; i < dtDonVi.Rows.Count; i = i + SoCot)
{
                                ...
                                <tr>
                                    <%for (int c = 0; c < SoCot; c++)
                                      {
                                          ...
                                          {
                                              Checked = "";
                                              sTenDonVi = Convert.ToString(dtDonVi.Rows[i + c]["TenHT"]);
                                    ...
                                    <td align="center" style="width: 40px;">
                                        <input type="checkbox" value="<%=MaDonVi %>" <%=Checked %> check-group="DonVi"
          ...
          <td style="width: 40%">
                            ...
                            <table class="mGrid" style="width: 100%">
                                <tr>
                                    <th align="center" style="width: 40px;">
                                        <input type="checkbox" id="abc" onclick="CheckAllNganh(this.checked)" />
                                    </th>
                                    <%for (int c = 0; c < SoCot * 2 - 1; c++)
                                      {%>
                                    <th>
                                ...
                                <tr>
                                    <%for (int c = 0; c < SoCot; c++)
                                      {
                                          ...
                                          {
                                              strChecked = "";
                                              strsTen = Convert.ToString(dtNganh.Rows[i + c]["sTenNganh"]);
                                              MaNganh = Convert.ToString(dtNganh.Rows[i + c]["iID_MaNganh"]);
                                              for (int j = 0; j < arrMaNganh.Length; j++)
                                              {
                                                  if (MaNganh.Equals(arrMaNganh[j]))
                                                  {
                                    ...
                                    <td align="center" style="width: 40px;">
                                        <input type="checkbox" value="<%=MaNganh %>" <%=strChecked %> check-group="Nganh"
                                             id="iID_MaNganh" name="iID_MaNganh" />
                                    </td>
                                    <td align="left">
                                        <%=strsTen%>
                                    </td>
