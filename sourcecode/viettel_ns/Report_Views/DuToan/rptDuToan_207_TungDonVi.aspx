...
<body>
    <%
        ...
        var iID_MaDonVi = Convert.ToString(ViewData["iID_MaDonVi"]);
        var trinhky = (int)ViewData["trinhky"];
        DataTable dtDonVi = DuToan_ReportModels.dtNganh_NhaNuoc(MaND,"207");
        ...
        if (PageLoad == "1")
        {
            ...
            for (int i = 0; i < arrMaDonVi.Length; i++)
            {
                arrView[i] =
                    String.Format(
                        @"/rptDuToan_207_TungDonVi/viewpdf?iID_MaDonVi={0}&MaND={1}&trinhky={2}",
                        arrMaDonVi[i], MaND, trinhky);
                Chuoi += arrView[i];
        ...
        string BackURL = trinhky == 1 ?
           Url.Action("TrinhKy", "DuToan_Report"):
           Url.Action("Index", "DuToan_Report", new { sLoai = "1" });

        using (Html.BeginForm("EditSubmit", "rptDuToan_207_TungDonVi", new {ParentID=ParentID, trinhky}))
        {
    %>
     <div class="title_tong">
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                    <td>
                        <span>Báo cáo dự toán chi ngân sách quốc phòng (phần chi quản lý hành chính) năm <%=iNamLamViec %>
                           </span>
                    </td>
                                    ...
                                    <%for (int c = 0; c < SoCot; c++)
                                      {
                                          ...
                                          {
                                              strChecked = "";
                                              strsTen = Convert.ToString(dtDonVi.Rows[i + c]["sTenNganh"]);
                                              MaDonVi = Convert.ToString(dtDonVi.Rows[i + c]["iID_MaNganh"]);
