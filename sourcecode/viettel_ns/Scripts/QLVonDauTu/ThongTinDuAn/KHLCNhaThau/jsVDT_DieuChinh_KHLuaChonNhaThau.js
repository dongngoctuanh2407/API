...
function GetDanhSachDuToan(id) {
    $.ajax({
        url: "/QLVonDauTu/KHLuaChonNhaThau/LayDanhSachDuToanDieuChinhTheoDuAn",
        type: "POST",
        data: { iID: id },
        dataType: "json",
        cache: false,
...
function Luu() {
    if (CheckLoi()) {
        ...
        objKHLuaChonNhaThau.iID_ParentID = $("#iID_KHLuaChonNTID").val();
        objKHLuaChonNhaThau.iID_LCNhaThauGocID = $("#iID_LCNhaThauGocID").val();
        ...
        $("#" + TBL_DANH_SACH_GOI_THAU + " tbody tr").each(function () {
            arrGoiThau.push({
                ...
                iID_ParentID: $(this).find(".r_GoiThauGoc select").val(),
...
function ThemMoiGoiThau() {
    ...
    if (arrDuToanID.length > 0) {
        ...
        dongMoi += "<td class='r_GoiThauGoc'>" + CreateHtmlSelectGoiThauGoc() + "</td>";
...
function LoadDataGoiThauGoc() {
    var lcntParentId = $("#iID_KHLuaChonNTID").val();
    $.ajax({
        url: "/QLVonDauTu/KHLuaChonNhaThau/GetListGoiThauGoc",
        type: "POST",
        data: { lcntParentId: lcntParentId },
        dataType: "json",
        cache: false,
        success: function (data) {
            if (data != null) {
...
function CreateHtmlSelectGoiThauGoc(value) {
    var htmlOption = "";
    htmlOption += "<option value=''></option>";
    arr_GoiThauGoc.forEach(x => {
        if (value != undefined && value == x.iID_GoiThauID)
            htmlOption += "<option value='" + x.iID_GoiThauID + "' selected>" + x.sTenGoiThau + "</option>";
        else
            htmlOption += "<option value='" + x.iID_GoiThauID + "'>" + x.sTenGoiThau + "</option>";
    })
    return "<select class='form-control'>" + htmlOption + "</option>";
}
...
function LuuChiTietGoiThau() {
    ...
    $("#" + TBL_DANH_SACH_NGUON_VON_MODAL + " tbody  input[type='checkbox']:checked").each(function () {
        var thisDong = this.parentElement.parentElement;
