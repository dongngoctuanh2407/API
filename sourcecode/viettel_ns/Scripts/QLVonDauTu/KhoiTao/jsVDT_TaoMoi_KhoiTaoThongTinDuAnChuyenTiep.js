...
$(document).ready(function () {
    ...
    $("#drpDuAn").change(function () {
        if (this.value != "") {
            $("#sTenDuAn").val($("#iID_DuAnID :selected").html());
            $.ajax({
                url: "/QLVonDauTu/QLKhoiTaoThongTinDuAnChuyenTiep/LayThongTinChiTietDuAn",
                type: "POST",
                data: { id: this.value },
                dataType: "json",
                cache: false,
                success: function (data) {
                    if (data != null) {
                        var objDuAn = data.objDuAn;
                        var objQDDT = data.objQDDT;
                        var objDuToan = data.objDuToan;
                        // fill thong tin chi tiet du an
                        if (objDuAn != null) {
                            $("#sMaDuAn").val(objDuAn.sMaDuAn);
                            if (objDuAn.iID_ChuDauTuID != null)
                                $("#iID_ChuDauTuID").val(objDuAn.iID_ChuDauTuID);
                            if (objDuAn.iID_LoaiCongTrinhID != null)
                                trLoaiCongTrinh.setSelection([objDuAn.iID_LoaiCongTrinhID]);
                            if (objDuAn.iID_CapPheDuyetID != null)
                                $("#iID_CapPheDuyetID").val(objDuAn.iID_CapPheDuyetID);
                            if (objDuAn.iID_DonViQuanLyID != null)
                                $("#iID_DonViQuanLyID").val(objDuAn.iID_DonViQuanLyID);

                            $("#sKhoiCong").val(objDuAn.sKhoiCong);
                            $("#sKetThuc").val(objDuAn.sKetThuc);
                        }

                        // fill thong tin QDDT
                        if (objQDDT != null) {
                            $("#sSoQDDT").val(objQDDT.sSoQuyetDinh);
                            $("#dNgayPheDuyetQDDT").val(objQDDT.sNgayQuyetDinh);

                            $("#sCoQuanPheDuyetQDDT").val(objQDDT.sCoQuanPheDuyet);
                            $("#sNguoiKyQDDT").val(objQDDT.sNguoiKy);

                            $("#iID_QDDauTuID").val(objQDDT.iID_QDDauTuID);
                        }

                        // fill thong tin TKDT
                        if (objDuToan != null) {
                            $("#sSoQuyetDinhTKDT").val(objDuToan.sSoQuyetDinh);
                            $("#dNgayDuyetTKDT").val(objDuToan.sNgayQuyetDinh);

                            $("#sCoQuanPheDuyetTKDT").val(objDuToan.sCoQuanPheDuyet);
                            $("#sNguoiKyTKDT").val(objDuToan.sNguoiKy);

                            $("#iID_DuToanID").val(objDuToan.iID_DuToanID);
                        }
                    }
                },
                error: function (data) {
...
function RenderGridView(data) {
    var columns = [
        ...
        { sTitle: "Ngày phê duyệt", sField: "dNgayPheDuyetQDDT", iWidth: "200px", sTextAlign: "center" },
        ...
        { sTitle: "Ngày phê duyệt", sField: "dNgayPheDuyetTKDT", iWidth: "200px", sTextAlign: "center" },
    ...
    $.ajax({
        ...
        success: function (r) {
            if (r.bIsComplete) {
                ...
                alert("Khởi tạo dự án chuyển tiếp thất bại.");
                return false;
            }
        }
    });
...
function SuKienDbClickDongTable() {
    ...
    $("#ViewTable td").dblclick(function () {
        ...
        $("#drpLoaiNganSach").val(data.iID_LoaiNguonVonIDValue).trigger("change");
