...
function BangDuLieu_onCellAfterEdit(h, c) {
    ...
    //Be du lieu tu cot thuc hien sang san xuat 
    if (c == 3 || c == 4 || c == 5) {
        if (Bang_arrGiaTri[h][c + 3] == 0) {
            var GiaTri = Bang_arrGiaTri[h][c];
            Bang_GanGiaTriThatChoO(h, c + 3, GiaTri, 1);
            BangDuLieu_onCellAfterEdit(h, c + 3)
        }
        if (Bang_arrGiaTri[h][c + 7] == 0) {
            var GiaTri = Bang_arrGiaTri[h][c];
            Bang_GanGiaTriThatChoO(h, c + 7, GiaTri, 1);
            BangDuLieu_onCellAfterEdit(h, c + 7)
        }
        if (Bang_arrGiaTri[h][c + 11] == 0) {
            var GiaTri = Bang_arrGiaTri[h][c];
            Bang_GanGiaTriThatChoO(h, c + 11, GiaTri, 1);
            BangDuLieu_onCellAfterEdit(h, c + 11)
        }
    }
    if (c == 6 || c == 7 || c == 8) {
        if (Bang_arrGiaTri[h][c + 4] == 0) {
            var GiaTri = Bang_arrGiaTri[h][c];
            Bang_GanGiaTriThatChoO(h, c + 4, GiaTri, 1);
            BangDuLieu_onCellAfterEdit(h, c + 4)
        }
        if (Bang_arrGiaTri[h][c + 8] == 0) {
            var GiaTri = Bang_arrGiaTri[h][c];
            Bang_GanGiaTriThatChoO(h, c + 8, GiaTri, 1);
            BangDuLieu_onCellAfterEdit(h, c + 8)
        }
    }
}
...
function BangDuLieu_CapNhapCotSoSanh(h) {
    var TienDeNghi = Bang_arrGiaTri[h][8];
    var TienThucHien = Bang_arrGiaTri[h][5];
    var TienTrucThuoc = Bang_arrGiaTri[h][12];
    var TienCTC = Bang_arrGiaTri[h][16];
    if (TienThucHien != 0) {
        var SoSanh = TienDeNghi / TienThucHien * 100;
        Bang_GanGiaTriThatChoO(h, 9, SoSanh, 1);
        SoSanh = TienTrucThuoc / TienThucHien * 100;
        Bang_GanGiaTriThatChoO(h, 13, SoSanh, 1);
        SoSanh = TienCTC / TienThucHien * 100;
        Bang_GanGiaTriThatChoO(h, 17, SoSanh, 1);

    }
    else {
        Bang_GanGiaTriThatChoO(h, 9, 0, 1);
        Bang_GanGiaTriThatChoO(h, 13, 0, 1);
        Bang_GanGiaTriThatChoO(h, 17, 0, 1);
    }
}
...
function BangDuLieu_CapNhapHangTong() {
    for (c = 0; c < Bang_nC; c = c + 1) {
        if (Bang_arrMaCot[c].startsWith("rTien")) {
            ...
            var Tong = 0, rTien6 = 0, rTien7 = 0, rTien8 = 0, rTien9 = 0, rTien10 = 0, rTien11 = 0, rTien12 = 0, rTien13 = 0;
            for (h = 0; h < Bang_nH; h = h + 1) {
                ...
                if (!(strTT.indexOf(".") > 0) && strTT != "") {
                    ...
                    //loi nhuan
                    if (parseInt(strTT) == 8) {
                        rTien8 = getLoiNhuan() * rTien7;
                        Bang_GanGiaTriThatChoO(h, c, rTien8);
                    }
                    if (parseInt(strTT) == 9) {
                        rTien9 = Bang_arrGiaTri[h][c];
                    }
                    if (parseInt(strTT) == 10) {
                        rTien10 = rTien7 + rTien8 + rTien9;
                        Bang_GanGiaTriThatChoO(h, c, rTien10);
                    }
                    //thue
                    if (parseInt(strTT) == 11) {
                        rTien11 = getThueGTGT() * rTien10;
                        Bang_GanGiaTriThatChoO(h, c, rTien11);
                    }
                    if (parseInt(strTT) == 12) {
                        rTien12 = rTien10 + rTien11;
                        Bang_GanGiaTriThatChoO(h, c, rTien12);
                    }
                    if (parseInt(strTT) == 13) {
                        rTien13 = rTien10 + rTien11;
                        Bang_GanGiaTriThatChoO(h, c, rTien13);
                    }
