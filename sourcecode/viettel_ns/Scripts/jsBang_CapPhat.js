...
function BangDuLieu_onCellAfterEdit(h, c) {
    ...
    if (Bang_arrMaCot[c] != "iID_MaDonVi" && Bang_arrMaCot[c] != "sTenDonVi") {
        var tongcp = 0;
        var tongdc = 0;
        var tongct = 0;
...
function Clone_values() {
    for (var i = 0; i < Bang_nH; i++) {
        ...
        var csMaCotConLai = Bang_arrCSMaCot["rTuChi_ConLai"];
        var csMaCotCap = Bang_arrCSMaCot["rTuChi"];

        if (Bang_LayGiaTri(i, "rTuChi") == 0 &&
            Bang_LayGiaTri(i, "rTuChi_ConLai") != 0 &&
            Bang_LayGiaTri(i, "rTuChi_ConLai") != null) {

            Bang_GanGiaTriThatChoO_colName(i, "rTuChi", parseFloat(Bang_LayGiaTri(i, "rTuChi_ConLai")));
        }
    }
}
...
function BangDuLieu_onKeypress_F8() {
    ...
    var yes = confirm("F8: Đ/c chắc chắn muốn cấp toàn bộ số chỉ tiêu còn lại?");
    if (!yes)
        return;
