...
function BangDuLieu_ThemHangMoi(h, hGiaTri) {
    if (h != null && BangDuLieu_DuocSuaChiTiet) {
        if (Bang_arrLaHangCha[hGiaTri] == false) {
            ...
            var csnm = Bang_arrCSMaCot['sXauNoiMa'];
            Bang_arrMaHang[h] = "_" + Bang_arrGiaTri[h - 1][cs] + "_" + Bang_arrGiaTri[h - 1][csnm];
            Bang_GanGiaTriThatChoO(h, cs, Bang_arrGiaTri[h - 1][cs]);
            Bang_GanGiaTriThatChoO(h, csnm, Bang_arrGiaTri[h - 1][csnm]);
...
function BangDuLieu_onCellAfterEdit(h, c) {   
    ...
    if (c != 9 && c != 10 && c != 17) {
        ...
        BangDuLieu_CapNhapLaiHangConLai(h, c);
    }

}

function BangDuLieu_CapNhapLaiHangConLai(h, c) {
    var TongTC = 0, TongHV = 0, TongCongTC = 0, TongCongHV = 0;
    if (Bang_arrType[c] == 1) {
        if (BangDuLieu_CoCotDuyet && c >= Bang_arrMaCot.length - 2) {
            return;
        }
        ...
        while (Bang_arrCSCha[csCha] >= 0) {
            csCha = Bang_arrCSCha[csCha];
            TongTC = BangDuLieu_TinhTongHangCon(csCha, 11);
            TongHV = BangDuLieu_TinhTongHangCon(csCha, 12);
            TongCongTC = Bang_arrGiaTri[csCha - 1][11];
            TongCongHV = Bang_arrGiaTri[csCha - 1][12];
            var ConLaiTC = parseFloat(TongCongTC) - parseFloat(TongTC);
            var ConLaiHV = parseFloat(TongCongHV) - parseFloat(TongHV);
            var sMauSac_TuChoi = Bang_sMauSac_TuChoi;
            var sMauSac_ChuaDuyet = Bang_sMauSac_ChuaDuyet;
            var sMauSac_DaDuyet = Bang_sMauSac_DongY;
            var Thieu = "", Thua = "", Du = "";
            Bang_GanGiaTriThatChoO_colName(csCha, "rTuChi", ConLaiTC);
            Bang_GanGiaTriThatChoO_colName(csCha, "rHienVat", ConLaiHV);
            if (ConLaiTC < 0 || ConLaiHV < 0)
                Bang_GanGiaTriThatChoO_colName(csCha, "sMauSac", sMauSac_TuChoi);
            else if (ConLaiTC == 0 && ConLaiHV == 0)
                Bang_GanGiaTriThatChoO_colName(csCha, "sMauSac", sMauSac_DaDuyet);
            else
                Bang_GanGiaTriThatChoO_colName(csCha, "sMauSac", Bang_sMauSac_ChuaDuyet);
        }
    }
}
