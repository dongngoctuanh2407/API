...
//var Bang_Cell_OrRow = true;
var iiD_MaPhongBanDich = "";
//link trang phân cấp
...
function BangDuLieu_onLoad() {
    //if (Bang_arrType[Bang_arrCSMaCot["rDonViDN"]] == "1" && Bang_arrType[Bang_arrCSMaCot["rTongCong"]] == "1") {
    ...
    if (h != null && BangDuLieu_DuocSuaChiTiet) {
        if (Bang_arrLaHangCha[hGiaTri] == false) {
            ...
            for (var c = Bang_nC_Fixed; c < Bang_nC; c++) {
                if (Bang_arrMaCot[c] == "sGhiChu" || Bang_arrMaCot[c] == "sSoCT" || Bang_arrMaCot[c].startsWith("iID_MaDonVi") || Bang_arrMaCot[c] == "iID_MaPhongBanDich" || Bang_arrMaCot[c].startsWith("sTenDonVi")) {
            ...
            Bang_arrMaHang[h] = "_" + Bang_arrGiaTri[h - 1][cs] + "_";
...
function BangDuLieu_onCellAfterEdit(h, c) {
    if (h < Bang_nH - 1) {
        ...
        //Gan gia tri mac dinh cho cot ma phong ban dich        
        ...
        if (Bang_arrMaCot[c] == "bPhanCap" && Bang_arrLaHangCha[h] == false) {
            ...
            url = unescape(BangDuLieu_Url_PhanCap + '?iID_MaChungTu=' + iID_MaChungTu);
            window.open(url, '_blank');
        }

        if (Bang_arrMaCot[c] == "rChiTapTrung") {
            var crTuChi = Bang_arrCSMaCot["rTuChi"];
            var rTuChi = Bang_arrGiaTri[h][crTuChi];
            var rChiTapTrung = Bang_arrGiaTri[h][c];
            if (rChiTapTrung > rTuChi) {
                // Bang_GanGiaTriThatChoO(h, c, 0);
                //Bang_keys.fnSetFocus(h, 0);
                alert("Số chi tập trung không được vượt quá tự chi");
                Bang_GanGiaTriThatChoO_colName(h, "sMauSac", Bang_sMauSac_TuChoi);
            }
        }
        ...
        if (Bang_arrType[Bang_arrCSMaCot["rDonViDN"]] == "1" && Bang_arrType[Bang_arrCSMaCot["rTongCong"]] == "1") {
            var crTuChi = Bang_arrCSMaCot["rTuChi"];
            var rTuChi = Bang_arrGiaTri[h][crTuChi];
            var crDonViDN = Bang_arrCSMaCot["rDonViDN"];
            var rDonViDN = Bang_arrGiaTri[h][crDonViDN];
            
            cs = Bang_arrCSMaCot["rTongCong"];
            Bang_GanGiaTriThatChoO(h, cs, rTuChi + rDonViDN);
        }
    ...
    if (iLoai == "4") {
        tong = 0;
        //Gia tri cot tong loai phan cap la 2
        var TongCongTC = Bang_arrGiaTri[0][16]
        var TongBD = 0;
        for (var j = 15; j < 23; j++) {
            tong = 0;
            for (var i = 2; i < Bang_nH - 1; i++) {
                if (Bang_arrLaHangCha[i] == false) {
                    tong += Bang_arrGiaTri[i][j];
        ...
        Bang_GanGiaTriThatChoO(1, 16, TongCongTC - TongBD);
    ...
    if (Bang_arrType[c] == "1") {
        ...
        if (Bang_arrType[Bang_arrCSMaCot["rDonViDN"]] == "1" && Bang_arrType[Bang_arrCSMaCot["rTongCong"]] == "1") {
            var tongC = 0;
            var tc = Bang_arrCSMaCot["rTongCong"];
            for (var i = 2; i < Bang_nH - 1; i++) {
                if (Bang_arrLaHangCha[i] == false) {
                    tongC += Bang_arrGiaTri[i][tc];

                }
            }

            Bang_GanGiaTriThatChoO(0, tc, tongC);
            Bang_GanGiaTriThatChoO(Bang_nH - 1, tc, tongC);
            BangDuLieu_CapNhapLaiHangCha(h, tc);
        }
    }
    if (Bang_arrMaCot[c] == "rChiTapTrung") {
        Bang_GanGiaTriThatChoO(0, c, tong);
...
function Clone_values() {
    for (var i = 0; i < Bang_nH; i++) {
        var csMaCotNamTruoc = Bang_arrCSMaCot["rTuChi_NT"];
       
        var GiaTriTuChiNamTruoc = Bang_arrGiaTri[i][csMaCotNamTruoc];

        if (Bang_arrType[csMaCotNamTruoc] == "1" && Bang_LayGiaTri(i, "rTuChi_NT") != 0 && Bang_LayGiaTri(i, "rTuChi_NT") != null && Bang_LayGiaTri(i, "rTuChi") == 0) {
            Bang_GanGiaTriThatChoO_colName(i, "rTuChi", parseFloat(Bang_LayGiaTri(i, "rTuChi_NT")));
        }
    }
}
...
function BangDuLieu_onKeypress_F4() {
    ...
    if (Bang_arrType[csMaCotNamTruoc] == "1") {
        var yes = confirm("F4: Đ/c chắc chắn muốn sao chép số liệu dự toán năm trước?");
        if (!yes)
            return;
