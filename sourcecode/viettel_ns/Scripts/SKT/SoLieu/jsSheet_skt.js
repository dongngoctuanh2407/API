...
function BangDuLieu_onCellAfterEdit(h, c) {
    if (Bang_arrType[c] != "1")
        return;    
    if (Bang_arrMaCot[c] == "TuChi" || Bang_arrMaCot[c] == "HuyDong" || Bang_arrMaCot[c] == "MuaHang" || Bang_arrMaCot[c] == "HangNhap" || Bang_arrMaCot[c] == "PhanCap") {
        BangDuLieu_TinhTong(h, c);
    }
    else if (Bang_arrMaCot[c] == "TuChi_DV" || Bang_arrMaCot[c] == "HuyDong_DV" || Bang_arrMaCot[c] == "MuaHang_DV" || Bang_arrMaCot[c] == "HangNhap_DV" || Bang_arrMaCot[c] == "PhanCap_DV") {
...
function BangDuLieu_TinhTong(h, c) {
    ...
    var GiaTriHangNhap = Bang_arrGiaTri[h][csMaCotHangNhap];
    var GiaTriPhanCap = Bang_arrGiaTri[h][csMaCotPhanCap];

    if (Bang_arrType[csMaCotTuChi] == "1") {
        if (GiaTriTuChi - GiaTriTuChi_3 > 0) {
            Bang_GanGiaTriThatChoO(h, csMaCotTang, GiaTriTuChi - GiaTriTuChi_3);
            Bang_GanGiaTriThatChoO(h, csMaCotGiam, 0);
        }
        else{
            Bang_GanGiaTriThatChoO(h, csMaCotTang, 0);
            Bang_GanGiaTriThatChoO(h, csMaCotGiam, GiaTriTuChi_3 - GiaTriTuChi);
        }
    }
    else if (Bang_arrType[csMaCotMuaHang] == "1") {
        var Tong_2 = GiaTriMuaHang + GiaTriHangNhap + GiaTriPhanCap;
...
function BangDuLieu_TinhTong_DV(h, c) {
    ...
    var csMaCotHangNhap = Bang_arrCSMaCot["HangNhap_DV"];
    ...
    if (Bang_arrType[csMaCotTuChi] == "1") {
        ...
        var Tong_2 = GiaTriHangNhap + GiaTriMuaHang + GiaTriPhanCap;
        Bang_GanGiaTriThatChoO(h, csMaCotTongCong, Tong_2);        
    }
