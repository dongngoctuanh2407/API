...
/*
    author: longsam
    khoa su dung nut back cua trinh duyet
*/
function disableBackBrowser() {
    window.onload = function () {
        if (typeof history.pushState === "function") {
            history.pushState("longsam", null, null);
...
function jsInit_TextBox() {
    ...
    //{-----------------------------------    
    $('.sheet input:text[nhap-so="1"]').on("keyup", function () {
        return ValidateNumberKeyUp(this);
    });
    $('.sheet input:text[nhap-so="1"]').on("keypress", function (event) {
        return ValidateNumberKeyPress(this, event);
    });
    $('.sheet input:text[nhap-so="1"]').on("focusout", function () {
    ...
    //{-----------------------------------
    $('.sheet input:text').on("keypress", function (event) {
        if (event.which == 13) {
    ...
    //{-----------------------------------
    $('.sheet input:text').on("focus", function () {
        var thisID = $(this).attr("id");
    ...
    //{-----------------------------------
    $('.sheet input:submit').on("click", function () {
...
function jsInit_Autocomplete() {
    var controls = $('.sheet input[autocomplete-control="1"]');
    var i;
