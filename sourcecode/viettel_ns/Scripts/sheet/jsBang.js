...
var Bang_Scroll_Size = 5; //Chiều rộng của cột tiêu đề
...
function Bang_txtONhapDuLieu_Autocomplete_onSource(txt, request, response) {
    ...
    if (h != null && c != null) {
        ...
        if (Truong == "iID_MaMucLucNganSach") {
            ...
            $.getJSON(url, { Truong: Truong, GiaTri: request.term, DSGiaTri: Bang_GhepGiaTriCuaTruong("sMaNoiDungChiCha", "iID_NoiDungChi") }, response);
        }
        else if (Truong == "sTenNganSach") {
            $.getJSON(url, { Truong: Truong, GiaTri: request.term, DSGiaTri: Bang_GhepGiaTriCuaTruong("sTenNganSach", "iID_NguonVonID") }, response);
        }
        else if (Truong == "sDuAnCha") {
            $.getJSON(url, { Truong: Truong, GiaTri: request.term, DSGiaTri: Bang_GhepGiaTriCuaTruongDuAnCha("sDuAnCha", "iID_KeHoach5Nam_DeXuat_ChiTietID") }, response);
        }
        else if (Truong == "sTenLoaiCongTrinh") {
            $.getJSON(url, { Truong: Truong, GiaTri: request.term, DSGiaTri: Bang_GhepGiaTriCuaTruong("sTenLoaiCongTrinh", "sMaLoaiCongTrinh") }, response);
        }
        else if (Truong == "sTenCDT") {
            $.getJSON(url, { Truong: Truong, GiaTri: request.term, DSGiaTri: Bang_GhepGiaTriCuaTruong("sTenCDT", "sId_CDT") }, response);
        }
...
function Bang_LayTenTruongVaTruongGan(c) {
    ...
    else if (Bang_arrMaCot[c].startsWith("sDonViThucHienDuAn")) {
        Truong = 'sDonViThucHienDuAn';
        TruongGan = 'iID_MaDonVi';
    }
    ...
    } else if (Bang_arrMaCot[c].startsWith("sTenKhoi")) {
        Truong = 'sTenKhoi';
        TruongGan = 'sMaKhoi';
    }
    ...
    if (Bang_arrMaCot[c].endsWith("_Co")) {
        ...
        Truong = 'iNhom';
        TruongGan = 'iNhom';
    } else if (Bang_arrMaCot[c].startsWith("TenDonVi")) {
        Truong = 'TenDonVi';
        TruongGan = 'iID_MaDonVi';
    } else if (Bang_arrMaCot[c].startsWith("sTenPhongBan")) {
        Truong = 'sTenPhongBan';
        TruongGan = 'sMaPhongBan';
    }
    else if (Bang_arrMaCot[c].startsWith("sMaNguonCha")) {
        Truong = 'sMaNguonCha';
        TruongGan = 'iID_Nguon';
    }
    else if (Bang_arrMaCot[c].startsWith("sMaNoiDungChiCha")) {
        Truong = 'sMaNoiDungChiCha';
        TruongGan = 'iID_NoiDungChi';
    }
    else if (Bang_arrMaCot[c].startsWith("sMaNguon")) {
        Truong = 'sMaNguon';
        TruongGan = 'iID_Nguon';
    }
    else if (Bang_arrMaCot[c].startsWith("sTenNganSach")) {
        Truong = 'sTenNganSach';
        TruongGan = 'iID_NguonVonID';
    }
    else if (Bang_arrMaCot[c].startsWith("sDuAnCha")) {
        Truong = 'sDuAnCha';
        TruongGan = 'iID_KeHoach5Nam_DeXuat_ChiTietID';
    }
    else if (Bang_arrMaCot[c].startsWith("sTenLoaiCongTrinh")) {
        Truong = 'sTenLoaiCongTrinh';
        TruongGan = 'sMaLoaiCongTrinh';
    }
    else if (Bang_arrMaCot[c].startsWith("sTenCDT")) {
        Truong = 'sTenCDT';
        TruongGan = 'sId_CDT';
    } else if (Bang_arrMaCot[c].startsWith("sLoaiNganSach")) {
        Truong = 'sLoaiNganSach';
        TruongGan = 'iLoaiNganSach';
    }
...
function Bang_onCellAfterEdit(h, c) {
    ...
    if (Bang_arrType[c] == "3") {
        ...
        if ("sLNS,sL,sK,sM,sTM,sTTM,sNG,sTNG".indexOf(Bang_arrMaCot[c]) >= 0) {
            ...
            url += '&DSGiaTri=' + Bang_GhepGiaTriCuaTruong("sLoaiNganSach", "iLoaiNganSach");
        }
        url = unescape(url);
...
var current_yTop = 0;
var last_scroll_date = null;
var scroll_counter = 0;

// status scrolling
var scrolling = false;

Bang_fnScroll = function (e) {
    if (!scrolling) {
        scrolling = true;
    ...
    ////if ($.browser.chrome) {
    //console.log("chrome:" + $('#' + BangDuLieuID_Slide_Div).scrollTop());
    ////}
    ////else {
    ////    console.log("firefix:" + $('#' + BangDuLieuID_Slide_Div).scrollTop());
    ////}
    ...
    //var step = 50;
    //if (yTop > current_yTop && yTop - current_yTop > step) {
    //    yTop = current_yTop + step;
    //}

    //else if (yTop < current_yTop && current_yTop - yTop > step) {
    //    yTop = current_yTop - step;
    //}
    ...
    //if (last_scroll_date != null) {
    //    deltaTime = now - last_scroll_date;
    //}
    //console.log("scrollerrrrr:" + (yTop - current_yTop));
    ...
    //    current_yTop = yTop;
    //    last_scroll_date = now;
    //}
    //else {

    //    $('#' + BangDuLieuID_Slide_Div).scrollTop(current_yTop);
    //    console.log(e);
    //}
}
...
//Hàm ghép mã của một trường
//sTruongTen là trường nhập trường mã là trường cần ghép
function Bang_GhepGiaTriCuaTruongDuAnCha(sTruongTen, sTruongMa) {
    var vR = "";
    var cs;
    var h = Bang_keys.Row();
    var c = Bang_keys.Col();
    if (Bang_arrMaCot[c] == sTruongTen) {
        ...
        var cID_KeHoach5NamID = Bang_arrCSMaCot["iID_KeHoach5Nam_DeXuatID"];
        vR += ",";
        vR += Bang_arrGiaTri[h][cID_KeHoach5NamID];
    }
    return vR;
}