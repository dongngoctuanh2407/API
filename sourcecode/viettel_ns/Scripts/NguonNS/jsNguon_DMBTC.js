...
function BangDuLieu_ThemHangMoiCungCap(h, hGiaTri) {
    if (h != null && Bang_arrLaHangCha[hGiaTri] == false) {
        ...
        Bang_GanGiaTriThatChoO(h, 2, "");        
        Bang_GanGiaTriThatChoO(h, 3, Bang_arrGiaTri[hGiaTri][5] + '-');
        Bang_GanGiaTriThatChoO(h, 4, "");
        Bang_GanGiaTriThatChoO(h, 5, Bang_arrGiaTri[hGiaTri][5]);
        Bang_GanGiaTriThatChoO(h, 6, Bang_arrGiaTri[hGiaTri][6]);
        Bang_GanGiaTriThatChoO(h, 7, "");
        Bang_GanGiaTriThatChoO(h, 8, Bang_arrGiaTri[hGiaTri][8]);
        Bang_arrMaHang[h] = "_";
...
function BangDuLieu_ThemHangMoiCon(h, hGiaTri) {
    if (h != null && Bang_arrLaHangCha[hGiaTri] == false) {
        ...
        Bang_GanGiaTriThatChoO(h, 4, "");
        Bang_GanGiaTriThatChoO(h, 5, Bang_arrGiaTri[hGiaTri][3]);
        Bang_GanGiaTriThatChoO(h, 6, false);
        Bang_GanGiaTriThatChoO(h, 7, "");
        Bang_GanGiaTriThatChoO(h, 8, Bang_arrGiaTri[hGiaTri][7]);
        Bang_arrMaHang[h] = "_";
        Bang_keys.fnSetFocus(h, 0);
    }
}

function jsDMBTC_LoadLaiChiTiet() {
    if ($("#tabs-1").css("display") != "none") {
        var url = jsDMBTCEdit_Url_Frame;
        ...
        jsDMBTC_Search_inteval = setInterval(function () {
            jsDMBTC_Search_clearInterval();
...
var jsDMBTC_Search_inteval = null;

function jsDMBTC_Search_onkeypress(e) {
    jsDMBTC_Search_clearInterval();
    if (e.keyCode == 13) {
        ...
        jsDMBTC_LoadLaiChiTiet();
    }
    else {
        jsDMBTC_Search_inteval = setInterval(function () { jsDMBTC_Search_clearInterval(); jsDMBTC_LoadLaiChiTiet(); }, 2000);
    }
}

function jsDMBTC_Search_clearInterval() {
    clearInterval(jsDMBTC_Search_inteval);
}