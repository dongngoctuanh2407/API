...
function BangDuLieu_onKeypress_F2() {
    var yes = confirm("F2: Đ/c chắc chắn muốn chọn toàn bộ mục lục nhu cầu hiện tại hiện tại?");
    if (!yes)
        return;
...
function BangDuLieu_onKeypress_F3() {
    var yes = confirm("F3: Đ/c chắc chắn muốn bỏ chọn toàn bộ mục lục nhu cầu hiện tại?");
    if (!yes)
        return;
...
function BangDuLieu_onCellAfterEdit(h, c) {
    ...
    var iID_MaMucLucNganSach = Bang_arrGiaTri[h][Bang_arrCSMaCot["Id"]];
    for (var i = 0; i < Bang_nH; i++) {
        if (Bang_arrGiaTri[i][Bang_arrCSMaCot["Id_Parent"]] == iID_MaMucLucNganSach) {
