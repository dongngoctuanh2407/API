...
{
    public class CapPhat_ChungTu_SheetTable : SheetTable
    {
        ...
        public CapPhat_ChungTu_SheetTable()
        {

        }

        public CapPhat_ChungTu_SheetTable(string id, int option, Dictionary<string, string> query)
        {
        ...
        private DataTable getDataChiTiet(CP_CapPhat entity)
        {
            var sql = @"
select  
        sLNS1,sLNS3,sLNS5,sLNS,sL,sK,sM,sTM,sMoTa,sXauNoiMa
        ,sXauNoiMa_Cha=''     
        ,bLaHangCha = CONVERT(bit,case sTM when '' then 'True' else 'False' end)
        ,iID_MaDonVi =''
        ,sTenDonVi   =''
...
where   sTTM=''
        and (sM<>'' or sL='')
        and (len(sLNS)<7 or (sLNS in (@lns) and @search))
...
";
            ...
            using (var con = ConnectionFactory.Default.GetConnection())
            {
                ...
                rows_dutoan.ForEach(r =>
                {
                    ...
                    if (row_mucluc != null)
                    {
                        row.ItemArray = new object[] {
                            ...
                            r["rTuChi_PhanBo"],
                            r["rTuChi_DaCap"],
                            Convert.ToDouble(r["rTuChi_PhanBo"]) - Convert.ToDouble(r["rTuChi_DaCap"]) - Convert.ToDouble(r["rTuChi"] == DBNull.Value ? 0 : r["rTuChi"]),
                            r["rTuChi"],
                        };
                ...
                // neu cap phat cho 1 don vi, thi loc hien thi cho 1 don vi thoi
                if (_entity.iID_MaDonVi.IsNotEmpty())
                {
                    var rows =
                        dt.AsEnumerable()
                            .Where(r => r.Field<bool>(ColumnNameIsParent) && r.Field<string>("sTM").IsNotEmpty()).ToList();
                    rows.ForEach(dt.Rows.Remove);

                    // hieu chinh lai dong con
                    rows = dt.AsEnumerable().Where(r => !r.Field<bool>(ColumnNameIsParent)).ToList();
                    rows.ForEach(r =>
                    {
                        var xauNoiMa_List = r.Field<string>("sXauNoiMa_Cha").ToList("-");
                        xauNoiMa_List.RemoveAt(xauNoiMa_List.Count - 1);
                        r["sXauNoiMa_Cha"] = xauNoiMa_List.Join("-");
                    });
                }
...
";
        ...
        {
            var sql = FileHelpers.GetSqlQuery("cp_sheet_tm.sql");
            using (var con = ConnectionFactory.Default.GetConnection())
            {
                var dt = con.GetTable(sql, new
                {
