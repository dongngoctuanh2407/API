...
SELECT		iID_MaDonVi,
			C1      = case when sLNS like '2%' then SUM(rTuChi+rHangMua+rHangNhap+rChiTaiNganh+rDuPhong) else 0 end,
			C2      = case when sLNS like '4%' then SUM(rTuChi+rHangMua+rHangNhap) else 0 end
...
SELECT		iID_MaDonVi,
			C1      = case when sLNS like '2%' then SUM(rTuChi) else 0 end,
			C2      = case when sLNS like '4%' then SUM(rTuChi) else 0 end
