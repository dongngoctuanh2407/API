...
SELECT		Ma_Dv = LEFT(iID_MaDonVi, 2), iID_MaPhongBanDich, sLNS, sL, sK, sM, sTM, sTTM, sNG,
			C1      = case when sNG = '01' then sum(rTuChi) else 0 end,
			C2      = case when sNG = '04' then sum(rTuChi) else 0 end,
			C3      = case when sNG = '21' then sum(rTuChi) else 0 end,
			C4      = case when sNG = '22' then sum(rTuChi) else 0 end,
			C5      = case when sNG = '23' then sum(rTuChi) else 0 end,
			C6      = case when sNG = '30' then sum(rTuChi) else 0 end,
			C7      = case when sNG = '31' then sum(rTuChi) else 0 end,
			C8      = case when sNG = '32' then sum(rTuChi) else 0 end,
			C9      = case when sNG = '37' then sum(rTuChi) else 0 end,
			C10     = case when sNG = '38' then sum(rTuChi) else 0 end,
			C11     = case when sNG = '40' then sum(rTuChi) else 0 end,
			C12     = case when sNG = '51' then sum(rTuChi) else 0 end,
			C13     = case when sNG = '53' then sum(rTuChi) else 0 end,
			C14     = case when sNG = '54' then sum(rTuChi) else 0 end,
			C15     = case when sNG = '55' then sum(rTuChi) else 0 end,
			C16     = case when sNG = '56' then sum(rTuChi) else 0 end,
			C17     = case when sNG = '67' then sum(rTuChi) else 0 end,
			C18     = case when sNG = '69' then sum(rTuChi) else 0 end,
			C19     = case when sNG = '76' then sum(rTuChi) else 0 end		
