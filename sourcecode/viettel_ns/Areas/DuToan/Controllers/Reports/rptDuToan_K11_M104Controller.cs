...
{
    public class rptDuToan_K11_M104ViewModel
    {
...
{
    public class rptDuToan_K11_M104Controller : FlexcelReportController
    {
        #region var def
        public string _viewPath = "~/Areas/DuToan/Views/Report/";
        private const string _filePath = "~/Report_ExcelFrom/DuToan/KiemTra/rptDuToan_K11_M104.xls";
            ...
            dtTemp.AsEnumerable().ToList()
                .ForEach(r =>
                {
                    ...
                    else
                    {
                        ...
                        {
                            row["TuChi2"] = dtDuToan.AsEnumerable()
                                    .Where(x => x.Field<string>("sNG") == nganhNghiepVu)
                                    .Sum(x => x.Field<decimal>("rTuChi"));

                            row["HangNhap2"] = dtDuToan.AsEnumerable()
                                      .Where(x => x.Field<string>("sNG") == nganhNghiepVu)
                                      .Sum(x => x.Field<decimal>("rHangNhap"));

                            row["HangMua2"] = dtDuToan.AsEnumerable()
                                     .Where(x => x.Field<string>("sNG") == nganhNghiepVu)
                                     .Sum(x => x.Field<decimal>("rHangMua"));
                        }
        ...
        {
            var sql = FileHelpers.GetSqlQuery("rptDuToan_K11_M104_dtkt.sql");
        ...
        {
            var sql = FileHelpers.GetSqlQuery("rptDuToan_K11_M104_dt.sql");
