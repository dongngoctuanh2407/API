...
{
    public class rptSKT_TrinhKy_K1BVController : FlexcelReportController
    {
        #region var def
        public string _viewPath = "~/Areas/SKT/Views/Reports/TrinhKy/";
        private string _filePath = "~/Report_ExcelFrom/SKT/TrinhKy/rptTrinhKy_K1BV.xls";
        ...
        public ActionResult Index()
        {
            ...
            var view = _viewPath + "rptTrinhKy_K1BV.cshtml";
            return View(view, vm);
        }
        ...
        {
            #region get data
            var donvis = (',' + _SKTService.Get_DonVisGBV_TheoBql_ExistData(PhienLamViec.iNamLamViec, PhienLamViec.iID_MaDonVi, PhienLamViec.iID_MaPhongBan == "02" ? "06,07,08,10" : PhienLamViec.iID_MaPhongBan, _id_phongban)
                                .AsEnumerable().Select(x => x.Field<string>("Id")).Join(","))
                                .Split(',').Skip((_to - 1) * (_columnCount)).Take(_columnCount).Join(",");
            using (var conn = ConnectionFactory.Default.GetConnection())
            using (var cmd = new SqlCommand("skt_report_k1bv", conn))
            {
        ...
        public JsonResult Ds_To(string id_PhongBan)
        {
            ...
            var count = _SKTService.Get_DonVisGBV_TheoBql_ExistData(PhienLamViec.iNamLamViec, PhienLamViec.iID_MaDonVi, PhienLamViec.iID_MaPhongBan == "02" ? "06,07,08,10" : PhienLamViec.iID_MaPhongBan, id_PhongBan).AsEnumerable().Count();
